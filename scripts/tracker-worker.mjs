...
const VERSION = "v10.6-priority-investment-diary";
...
function alertMessage(record, hit){
  ...
  const lines = [
    ...
    "",
    `Target 1: $${fmtPrice(record.tp1)} (${ag.tp1Pct.toFixed(2)}%)`,
    `Target 2: $${fmtPrice(record.tp2)} (${ag.tp2Pct.toFixed(2)}%)`,
    `Stop / invalid: $${fmtPrice(record.invalidation)} (${ag.riskPct.toFixed(2)}%)`,
    "",
