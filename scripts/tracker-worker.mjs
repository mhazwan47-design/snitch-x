...
const VERSION = "v7.1-prime-category-full-worker";
...
async function loadDb(){
  try{
    ...
    return Array.isArray(db.records)?db.records:[];
  }catch{return []}
}
async function saveDb(records,meta){
  await fs.mkdir(path.dirname(OUT),{recursive:true});
  await fs.writeFile(OUT,JSON.stringify({version:VERSION,updatedAt:new Date().toISOString(),meta,records:records.sort((a,b)=>new Date(b.lastUpdated||0)-new Date(a.lastUpdated||0))},null,2));
}

async function main(){
  const old=await loadDb();
  ...
  for(const token of RADAR){
    try{
      ...
      for(const item of ranked){
        ...
        if(existing){map.set(item.id,updateRec(existing,item)); updated++}
        else{map.set(item.id,recFrom(item)); discovered++}
      }
  ...
  for(const r of [...map.values()].slice(0,180)){
    try{
      ...
      if(found){map.set(r.id,updateRec(r,found)); updated++}
      await new Promise(x=>setTimeout(x,200));
    }catch{}
  }

  await saveDb([...map.values()],{radarTokens:RADAR,discovered,updated,totalRecords:map.size});
  console.log(`SNITCH X DB updated. discovered=${discovered}, updated=${updated}, total=${map.size}`);
}
