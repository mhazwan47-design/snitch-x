...
const VERSION = "v10.3-beginner-signal-mode";
...
function alertMessage(record, hit){
  const sig = beginnerSignal(record, hit);
  ...
  if(record.signalType === "BUY"){
    lines.push(entryGuide(record));
    lines.push(`Target 1: $${fmtPrice(record.tp1)} ${action.toTP1 !== undefined ? `(${action.toTP1.toFixed(2)}%)` : ""}`);
    lines.push(`Target 2: $${fmtPrice(record.tp2)} ${action.toTP2 !== undefined ? `(${action.toTP2.toFixed(2)}%)` : ""}`);
    lines.push(`Stop / invalid: $${fmtPrice(record.invalidation)} ${action.toStop !== undefined ? `(${action.toStop.toFixed(2)}%)` : ""}`);
  } else {
    lines.push(`Sell zone: $${fmtPrice(record.zoneLow)} – $${fmtPrice(record.zoneHigh)}`);
    lines.push(`Bear trigger: $${fmtPrice(record.trigger)}`);
    lines.push(`Invalidation: $${fmtPrice(record.invalidation)}`);
  }
  lines.push("");
  lines.push(`<b>WHAT TO DO:</b> ${esc(sig.simpleAction)}`);
  lines.push(`<b>WHY:</b> ${esc(sig.why)}`);
  lines.push("");
  lines.push("Advanced:");
  lines.push(`Signal: ${esc(record.signalType)} · Category: ${esc(record.category)} · Security: ${esc(record.securityStatus || "UNKNOWN")} ${Number(record.securityScore||0).toFixed(0)}`);
  lines.push(`System hit: ${esc(hit)} · Hits: ${["ZN","TR","T1","T2","INV"].filter(k => ({ZN:record.hitZone,TR:record.hitTrigger,T1:record.hitTP1,T2:record.hitTP2,INV:record.hitInvalid}[k])).join(" ") || "-"}`);
