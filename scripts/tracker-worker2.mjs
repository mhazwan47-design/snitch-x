...
const VERSION = "v10.12.1-worker-async-patch";
...
function pruneActiveRecords(records){
  return (records||[]).filter(isActiveTradeValid).sort((a,b)=>new Date(b.lastUpdated||0)-new Date(a.lastUpdated||0)).slice(0,120);
}
...
async function saveDb(db,meta){
  ...
  const records = pruneActiveRecords(db.records||[]);
...
async function main(){
  ...
  await saveDb(db,{radarTokens:RADAR,discovered,updated,alerts,successes,totalRawRecords:map.size,totalActiveValidRecords:activeAfterPrune,totalSuccessForecasts:successAfterPrune,cleanup:"valid_active_plus_success_only"});
  console.log(`SNITCH X DB updated. discovered=${discovered}, updated=${updated}, alerts=${alerts}, successes=${successes}, activeValid=${activeAfterPrune}, success=${successAfterPrune}, raw=${map.size}`);
}
