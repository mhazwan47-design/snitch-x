...
const STORE = "snitch_x_v10_13_fresh_active_only";
...
function scannerItemsFromRecords(records){
  return freshActionRecords(records)
    .sort((a,b) => {
...
function merge(local,remote){const m=new Map();[...(remote||[]),...(local||[])].forEach(r=>{if(!r?.id)return;const e=m.get(r.id);if(!e)m.set(r.id,r);else m.set(r.id,new Date(r.lastUpdated||r.completedAt||0)>=new Date(e.lastUpdated||e.completedAt||0)?{...e,...r}:{...r,...e})});return pruneRecordsForStorage([...m.values()]).sort((a,b)=>new Date(b.lastUpdated||b.completedAt||0)-new Date(a.lastUpdated||a.completedAt||0))}
...
function isRecordSuccess(r){
  return !!(r?.hitTP1 || r?.hitTP2 || String(r?.resultStatus||"").includes("SUCCESS") || String(r?.result||"").includes("WIN"));
}
function isRecordInvalidated(r){
  return !!(r?.hitInvalidation || r?.hitInvalid || String(r?.status||"").includes("INVALID") || String(r?.result||"").includes("FAILED"));
}
...
function isUsefulRecord(r){
  return isRecordSuccess(r) || isActiveTradeValid(r) || !!r?.isSuccessArchive;
}
function successForecastToRecord(x){
  const level = x.successLevel || (x.hitTP2 ? "TP2" : "TP1");
  return {
    ...x,
    id: `success-${x.id}-${level}`,
    originalId: x.id,
    isSuccessArchive: true,
    hitTP1: true,
    hitTP2: level === "TP2",
    resultStatus: level === "TP2" ? "SUCCESS_TP2" : "SUCCESS_TP1",
    status: level === "TP2" ? "SUCCESS TP2" : "SUCCESS TP1",
    result: level === "TP2" ? "WIN TP2" : "WIN TP1",
    lastUpdated: x.completedAt || x.lastUpdated || new Date().toISOString(),
    trackedAt: x.completedAt || x.trackedAt || x.lastUpdated || new Date().toISOString(),
    currentPrice: n(x.currentPrice || x.tp2 || x.tp1 || x.signalPrice),
    signalPrice: n(x.signalPrice),
  };
}

function remainingToTP1Pct(r){
...
function isFreshActionableNow(r){
  if(!isActiveTradeValid(r)) return false;
  const remain = remainingToTP1Pct(r);
  if(remain < MIN_REMAINING_TO_TP1_PCT) return false;
  const p=n(r.currentPrice || r.signalPrice), zLo=n(r.zoneLow), zHi=n(r.zoneHigh), tr=n(r.trigger);
  const isSell = r.signalType === "SELL";
  if(!(p>0)) return false;

  // Fresh list rule: current price must still make sense for the setup now.
  // BUY: price cannot be below invalidation or already above TP1.
  // SELL: price cannot be above invalidation or already below TP1.
  if(isSell){
  ...
  // If it is not zone/trigger active and too old, do not show it.
  if(!(r.hitZone || r.hitTrigger) && hoursOld(r.trackedAt || r.firstSeenAt || r.lastUpdated) > 12) return false;
  return true;
}
function freshActionRecords(records){
  return pruneRecordsForStorage(records).filter(isFreshActionableNow);
}

function pruneRecordsForStorage(records){
  const active = [];
  const success = [];
  for(const r of (records||[])){
    if(!r?.id) continue;
    if(r.isSuccessArchive || isRecordSuccess(r)) success.push(r);
    else if(isActiveTradeValid(r)) active.push(r);
  }
  const byId = new Map();
  [...active, ...success].forEach(r => {
    const prev = byId.get(r.id);
    if(!prev || new Date(r.lastUpdated||r.completedAt||0) > new Date(prev.lastUpdated||prev.completedAt||0)) byId.set(r.id,r);
  });
  const out = [...byId.values()].sort((a,b)=>new Date(b.lastUpdated||b.completedAt||0)-new Date(a.lastUpdated||a.completedAt||0));
  const keptActive = out.filter(isActiveTradeValid).slice(0,120);
  const keptSuccess = out.filter(r=>r.isSuccessArchive || isRecordSuccess(r)).slice(0,250);
  return [...keptActive, ...keptSuccess].sort((a,b)=>new Date(b.lastUpdated||b.completedAt||0)-new Date(a.lastUpdated||a.completedAt||0));
}
function validActionRecords(records){
  return freshActionRecords(records);
}
function usefulLabRecords(records){
  return pruneRecordsForStorage(records).filter(isUsefulRecord);
}
...
const CEX_TOKEN_SET = new Set(["1000CAT","1000SATS","AAVE","ACT","ADA","AEVO","AI","AIXBT","ALGO","ALT","APE","APT","ARB","ARKM","ATOM","AVAX","BCH","BERA","BLUR","BNB","BOME","BONK","BTC","CAKE","CETUS","CHZ","COMP","CRV","DEEP","DOGE","DOGS","DOT","DYDX","EIGEN","ENA","ENS","ETC","ETH","FET","FIL","FLOKI","GALA","GMT","GMX","GOAT","GRASS","HBAR","ICP","IMX","INJ","JASMY","JST","JTO","JUP","KAITO","KAS","LDO","LINK","LQTY","LTC","MANA","MASK","MATIC","MEME","MEW","MINA","MKR","MOG","NEAR","NOT","ONDO","OP","ORDI","PENDLE","PENGU","PEPE","PNUT","POL","POPCAT","PYTH","RENDER","RNDR","ROSE","RUNE","SAND","SEI","SHIB","SOL","STRK","STX","SUI","SUSHI","TAO","TIA","TON","TRX","TURBO","UNI","USDC","USDT","VET","VIRTUAL","WIF","WLD","XRP","ZK","ZRO"]);

const CEX_DIRECT_TRADE_SET = new Set(["1000CAT","1000SATS","AAVE","ACT","ADA","AEVO","AI","AIXBT","ALGO","ALT","APE","APT","ARB","ARKM","ATOM","AVAX","BCH","BERA","BLUR","BNB","BOME","BONK","BTC","CAKE","CETUS","CHZ","COMP","CRV","DEEP","DOGE","DOGS","DOT","DYDX","EIGEN","ENA","ENS","ETC","ETH","FET","FIL","FLOKI","GALA","GMT","GMX","GOAT","GRASS","HBAR","ICP","IMX","INJ","JASMY","JST","JTO","JUP","KAITO","KAS","LDO","LINK","LQTY","LTC","MANA","MASK","MATIC","MEME","MEW","MINA","MKR","MOG","NEAR","NOT","ONDO","OP","ORDI","PENDLE","PENGU","PEPE","PNUT","POL","POPCAT","PYTH","RENDER","RNDR","ROSE","RUNE","SAND","SEI","SHIB","SOL","STRK","STX","SUI","SUSHI","TAO","TIA","TON","TRX","TURBO","UNI","USDC","USDT","VET","VIRTUAL","WIF","WLD","XRP","ZK","ZRO"]);
...
function tokenRiskBucket(record){
  const token = String(record?.token || String(record?.pairLabel||'').split('/')[0] || '').toUpperCase();
  const pair = String(record?.pairLabel || '').toUpperCase();
  const cat = String(record?.category || '').toUpperCase();
  const highVol = new Set(['PEPE','WIF','BONK','FLOKI','SHIB','1000SATS','1000CAT','ACT','PNUT','MEME','TURBO','BOME','MEW','POPCAT','MOG','GOAT','PENGU','DOGS','NOT']);
  const narrative = new Set(['AI','AIXBT','VIRTUAL','GRASS','KAITO','BERA','ZRO','AEVO','EIGEN','ALT','ARKM','CETUS','DEEP','FET','RNDR','RENDER','WLD','ONDO','ENA']);
  if(cat.includes('NEW') || pair.includes('/NEW')) return 'NEW PAIR';
  if(highVol.has(token)) return 'HIGH VOL';
  if(narrative.has(token)) return 'NARRATIVE';
  if(CEX_TOKEN_SET.has(token)) return 'CEX SAFE';
  return 'WILD CARD';
}
function modeSizeMultiplier(mode,bucket){
  const m=String(mode||'EXTREME').toUpperCase();
  if(m==='BALANCED') return bucket==='HIGH VOL'||bucket==='NEW PAIR'?0.35:0.65;
  if(m==='AGGRESSIVE') return bucket==='HIGH VOL'||bucket==='NEW PAIR'?0.55:0.85;
  return bucket==='HIGH VOL'||bucket==='NEW PAIR'?0.70:1.00;
}
function hunterSize(capital,label,bucket,mode){
  const cap=n(capital,10); const a=String(label||''); let pct=0;
  if(a.includes('GRAB')) pct=.70; else if(a.includes('ATTACK')) pct=.45; else if(a.includes('HIGH VOL')) pct=.25; else if(a.includes('NEW PAIR')) pct=.15; else if(a.includes('SCOUT')) pct=.20; else if(a.includes('ENTER')) pct=.50;
  return Math.max(0,Math.min(cap,Math.round(cap*pct*modeSizeMultiplier(mode,bucket)*100)/100));
}

function aggressivePriorityForRecord(record, capital=10, mode='EXTREME'){
  const isSell = record.signalType === 'SELL';
  const current = n(record.currentPrice || record.signalPrice);
  const tp1 = n(record.tp1);
  const tp2 = n(record.tp2);
  ...
  const bucket = tokenRiskBucket(record);
  ...
  if(isSell) return {agLabel:'SKIP / SELL WATCH', agSize:0, agScore:0, agReason:'Sell watch. Do not buy spot.', agTP1:tp1Pct, agTP2:tp2Pct, agRisk:riskPct, bucket};
  if(record.hitInvalid || record.hitInvalidation) return {agLabel:'KILL / INVALID', agSize:0, agScore:0, agReason:'Invalidation already hit.', agTP1:tp1Pct, agTP2:tp2Pct, agRisk:riskPct, bucket};
  if(record.hitTP2) return {agLabel:'TAKE PROFIT / TOO LATE', agSize:0, agScore:0, agReason:'TP2 already hit. Do not chase.', agTP1:tp1Pct, agTP2:tp2Pct, agRisk:riskPct, bucket};
  if(record.hitTP1 && tp2Pct < 2.5) return {agLabel:'CHASE WARNING', agSize:0, agScore:0, agReason:'TP1 already hit and remaining room is small.', agTP1:tp1Pct, agTP2:tp2Pct, agRisk:riskPct, bucket};

  let score = 0;
  score += sec >= 100 ? 25 : sec >= 90 ? 18 : sec >= 75 ? 8 : sec >= 60 ? 2 : -20;
  ...
  score += tp2Pct >= 12 ? 22 : tp2Pct >= 10 ? 18 : tp2Pct >= 7 ? 14 : tp2Pct >= 4 ? 8 : 0;
  score += riskPct <= 3 ? 12 : riskPct <= 6 ? 8 : riskPct <= 10 ? 3 : riskPct <= 18 ? -2 : -10;
  score += bucket==='CEX SAFE' ? 8 : bucket==='NARRATIVE' ? 7 : bucket==='HIGH VOL' ? 5 : bucket==='NEW PAIR' ? 3 : 0;

  let label='WATCH ONLY', reason='Opportunity exists but not strong enough for action.';
  if(record.hitTrigger && sec >= 90 && tp2Pct >= 4){
    label='GRAB NOW'; reason='Trigger confirmed, security strong, remaining TP2 still attractive.';
  } else if((record.hitZone || cat.includes('GOLDEN')) && sec >= 85 && tp2Pct >= 6){
    label='ATTACK ENTRY'; reason='Aggressive opportunity: strong category and enough upside.';
  } else if(sec >= 75 && tp2Pct >= 7 && riskPct <= 14){
    label='SCOUT ENTRY'; reason='Candidate has upside, but no trigger/zone confirmation yet.';
  }

  if(m==='EXTREME'){
    if(bucket==='HIGH VOL' && sec>=65 && tp2Pct>=7 && riskPct<=28){label='HIGH VOL SCOUT'; reason='High volatility setup. Small fast scout only.'; score=Math.max(score,72);}
    if(bucket==='NARRATIVE' && sec>=65 && tp2Pct>=6 && riskPct<=25){label='ATTACK ENTRY'; reason='Narrative/momentum attack entry.'; score=Math.max(score,74);}
    if(bucket==='NEW PAIR' && sec>=60 && tp2Pct>=10 && riskPct<=35){label='NEW PAIR HUNTER'; reason='New pair hunter setup. Very risky; small size only.'; score=Math.max(score,68);}
  } else if(m==='AGGRESSIVE'){
    if((bucket==='HIGH VOL'||bucket==='NARRATIVE') && sec>=72 && tp2Pct>=8 && riskPct<=24){label=bucket==='HIGH VOL'?'HIGH VOL SCOUT':'ATTACK ENTRY'; reason='Aggressive mode allows this risk bucket.'; score=Math.max(score,68);}
  }

  const size = hunterSize(capital,label,bucket,mode);
  return {agLabel:label, agSize:size, agScore:Math.round(score), agReason:reason, agTP1:tp1Pct, agTP2:tp2Pct, agRisk:riskPct, bucket};
}


function splitPair(pairLabel){
  const raw = String(pairLabel || '').toUpperCase().replace('-', '/');
  const parts = raw.split('/');
  return { base: parts[0] || '', quote: parts[1] || '' };
}
function binanceTradeUrlForSymbol(symbol){
  return `https://www.binance.com/en/trade/${String(symbol||'').toUpperCase()}?type=spot`;
}
function routeQualityForPair(record){
  const {base, quote} = splitPair(record?.pairLabel);
  const token = String(record?.token || base || '').toUpperCase();
  const q = String(quote || '').toUpperCase();
  const stable = ['USDT','USDC','FDUSD','BUSD','DAI','USD'].includes(q);
  const majorQuote = ['WETH','WBTC','WBNB','ETH','BTC','BNB','SOL','WSOL'].includes(q);
  const routePair = stable ? `${token}/${q === 'USDC' ? 'USDC' : 'USDT'}` : `${token}/USDT`;
  const symbol = routePair.replace('/', '');
  if(stable && token){return {routeMode:'Binance Direct',routePair,routeWarning:'',routeUrl:binanceTradeUrlForSymbol(symbol),routePenalty:0,routeButton:`Open Binance ${routePair}`};}
  if(majorQuote && token){return {routeMode:'Binance Mirror',routePair,routeWarning:`Original scanner pair is ${record?.pairLabel}; Binance route opens ${routePair} as mirror.`,routeUrl:binanceTradeUrlForSymbol(symbol),routePenalty:80,routeButton:`Open Binance ${routePair} Mirror`};}
  return {routeMode:'Chart Only',routePair:record?.pairLabel || `${token}/USDT`,routeWarning:'No safe Binance spot mirror detected. Use chart first.',routeUrl:record?.url || '#',routePenalty:180,routeButton:'Open Chart'};
}
function routeForAnyHolding(pairLabel, token, originalUrl){return routeQualityForPair({ pairLabel, token, url: originalUrl });}
function routeBadgeText(routeMode){if(routeMode === 'Binance Direct') return 'Direct'; if(routeMode === 'Binance Mirror') return 'Mirror'; return 'Chart';}
function routeAdvisor(record, capital=10, mode='EXTREME'){
  ...
  const ag = aggressivePriorityForRecord(record, capital, mode);
  const rq = routeQualityForPair(record);
  let route = rq.routeMode || 'Chart Only'; let platform = route === 'Chart Only' ? 'DexScreener' : 'Binance'; let actionLabel = rq.routeButton || 'Open Chart'; let routeUrl = rq.routeUrl || record?.url || '#'; let viability = ag.agLabel || 'WATCH ONLY'; let reason = ag.agReason || 'Valid fresh candidate.';
  if(isLegacySecurity){return { ...pnl, ...beginnerLabelForRecord(record), ...ag, ...rq, route:'Chart Only', platform:'DexScreener', actionLabel:'Open Chart', routeUrl:record?.url || '#', viability:'WATCH ONLY', reason:'Legacy record missing security score. Run scanner/sync again before action.', feeTier:fee.feeTier };}
  if(isWait){return { ...pnl, ...beginnerLabelForRecord(record), ...ag, ...rq, route:'Chart Only', platform:'DexScreener', actionLabel:'Open Chart', routeUrl:record?.url || '#', viability:'WATCH ONLY', reason:'Category is WAIT. Keep monitoring; not suitable for action button yet.', feeTier:fee.feeTier };}
  if(secStatus === 'BLOCKED' || secStatus === 'HIGH RISK'){route = 'Avoid'; platform = 'DexScreener'; actionLabel = 'Open Chart Only'; routeUrl = record?.url || '#'; viability = 'AVOID'; reason = 'Security status is not suitable for action.';}
  else if(isSell){route = 'Exit / Avoid Buy'; platform = 'DexScreener'; actionLabel = 'Open Chart'; routeUrl = record?.url || '#'; viability = 'AVOID FRESH BUY'; reason = 'SELL WATCH detected. This is for protection/exit monitoring, not fresh buy.';}
  else if(listedStyle && (cap <= 50 || chain === 'ethereum')){viability = cap < 10 ? 'PAPER TRADE ONLY' : 'SMALL TEST OK'; reason = 'Major token style. For small capital, CEX spot is usually cleaner than DEX gas.';}
  else if(fee.feeScore >= 70){platform = 'DexScreener'; viability = pnl.tp1Profit < 0.20 && cap <= 20 ? 'PAPER TRADE ONLY' : 'SMALL TEST OK'; reason = fee.note;}
  if(rq.routeWarning) reason = `${reason} ${rq.routeWarning}`;
  return { ...pnl, ...beginnerLabelForRecord(record), ...ag, route, platform, actionLabel, routeUrl, viability, reason, feeTier:fee.feeTier, ...rq };
}
...
function orderPrice(v){
  const d=decimalsForPrice(v);
  return n(v).toFixed(d);
}
function practicalStopPct(advisory){
  const label=String(advisory?.agLabel||"");
  if(label.includes("GRAB")) return 1.25;
  if(label.includes("ENTER")) return 1.75;
  if(label.includes("SCOUT")) return 2.25;
  return 2.0;
}
function practicalLimitGapPct(entry){
  const x=n(entry);
  if(x >= 1000) return 0.20;
  if(x >= 1) return 0.30;
  return 0.45;
}
function orderSetupFromRecord(record, advisory, capital){
  const entry=n(record.currentPrice || record.signalPrice);
  const cap=Math.min(n(capital,10), n(advisory?.agSize, n(capital,10)) || n(capital,10));
  const qty=entry>0 ? cap/entry : 0;
  const stopPct=practicalStopPct(advisory);
  const gapPct=practicalLimitGapPct(entry);
  const practicalStop=entry*(1-stopPct/100);
  const stopLimit=practicalStop*(1-gapPct/100);
  const hardInvalid=n(record.invalidation);
  const tp1=n(record.tp1);
  const tp2=n(record.tp2);
  return {
    capital:cap,
    ...
    entry,
    hardInvalid,
    practicalStopPct:stopPct,
    stopTrigger:practicalStop,
    stopLimit,
    stopLimitGapPct:gapPct,
    tp1,
    tp2,
    tp1PartialQty: qty*0.5,
    tp2RemainderQty: qty*0.5,
    stopText:`SL Trigger: ${orderPrice(practicalStop)} | SL Price: ${orderPrice(stopLimit)} | Amount: MAX / ${qty.toPrecision(6)}`,
    tpText:`TP Limit: ${orderPrice(tp1)} for TP1 / partial | TP2 reference: ${orderPrice(tp2)}`
  };
}

function investmentFromRecord(record, advisory, capital){
  const setup = orderSetupFromRecord(record, advisory, capital);
  ...
  return {
    ...
    suggestedSize: advisory?.agSize || setup.capital,
    investedCapital: setup.capital,
    entryPrice: setup.entry,
    quantity: setup.qty,
    stopPrice: setup.hardInvalid,
    stopTrigger: setup.stopTrigger,
    stopLimit: setup.stopLimit,
    practicalStopPct: setup.practicalStopPct,
    stopLimitGapPct: setup.stopLimitGapPct,
    tp1: setup.tp1,
    tp2: setup.tp2,
    tp1PartialQty: setup.tp1PartialQty,
    tp2RemainderQty: setup.tp2RemainderQty,
...
function TradeActionCenter({ records, investments, onMarkInvestment, hunterMode="EXTREME" }){
  ...
  const rows = useMemo(() => {
    const mapped = validActionRecords(records).map(r => {
      const advisory = routeAdvisor(r, capital, hunterMode);
      advisory.priorityScore = tradePriorityScore(r, advisory, capital);
      return { ...r, advisory };
    });
    const bestByRoute = new Map();
    for (const row of mapped) {
      const key = row.advisory.routePair || row.pairLabel;
      const old = bestByRoute.get(key);
      if(!old || n(row.advisory.priorityScore) > n(old.advisory.priorityScore)) bestByRoute.set(key,row);
    }
    const base = [...bestByRoute.values()];
    let filtered = base;
  ...
  }, [records, capital, filter, sort, dir, hunterMode]);
  ...
  return (
    <section className="trade-center">
      <div className="lab-head">
        <div>
          <p className="eyebrow">Trade Action Center · v10.13 Fresh Valid Only</p>
          <h2>Best trade first, with Binance-ready order fields</h2>
          <p className="muted">
            Input modal once. SNITCH X ranks setups and shows the same field names you fill in Binance TP/SL.
          </p>
        </div>
      ...
      <BinanceTermsGuide />
      <RouteSafetyGuide />
      <div className="lab-note"><b>Validity gate:</b> active rows are rebuilt fresh. Passed TP, invalid, stale, dust, and low-room setups are removed from Scanner / Lab / Trade Action Center.</div>
      ...
      <section className="stats lab-stats">
        <div><span>Suggested List</span><b>{rows.length}</b></div>
        <div><span>Modal Input</span><b>${n(capital).toFixed(0)}</b></div>
        <div><span>Total Suggested Size</span><b>${totalSuggested.toFixed(0)}</b></div>
      ...
      <div className="lab-filters">
        <select value={filter} onChange={e=>setFilter(e.target.value)}>
          ...
          <option value="ALL">Valid Records</option>
      ...
      <div className="table-wrap">
        <table className="tracker-table action-table">
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr><td colSpan="13" className="empty-cell">No suggested trade list for this modal/filter.</td></tr>
            ) : rows.map((r) => {
              const a = r.advisory;
              ...
              return (
                <tr key={`${r.id}-action`}>
                  ...
                  <td><b>{a.routeMode || a.route}</b><small>{a.routePair || r.pairLabel}</small><small>{a.platform} · {a.feeTier}</small></td>
                  <td>{(()=>{const os=orderSetupFromRecord(r,a,capital); return <div className="binance-fields"><b>TP Limit {orderPrice(os.tp1)}</b><small>SL Trigger {orderPrice(os.stopTrigger)}</small><small>SL Price {orderPrice(os.stopLimit)}</small><small>Amount {os.qty.toPrecision(6)} {r.token}</small>{a.routeMode==="Binance Mirror" && <small className="route-warn">Mirror of {r.pairLabel}</small>}</div>})()}</td>
                  <td>
                    <a className="action-link" href={a.routeUrl} target="_blank" rel="noreferrer">{a.actionLabel}</a>
                    {a.routeMode === "Binance Direct" && <a className="action-link ghost" href={platformTradeUrl("OKX", r.token)} target="_blank" rel="noreferrer">OKX</a>}
                    {a.routeMode === "Binance Direct" && <a className="action-link ghost" href={platformTradeUrl("Bybit", r.token)} target="_blank" rel="noreferrer">Bybit</a>}
                    <a className="action-link ghost" href={r.url} target="_blank" rel="noreferrer">Original Chart</a>
                  </td>
                  <td>
...
function BinanceTermsGuide(){
  return <section className="binance-guide">
...
function RouteSafetyGuide(){
  return <section className="binance-guide route-guide">
    <div><b>Route Safety Guide</b><span>All buttons now show exact route type.</span></div>
    <div><b>Binance Direct</b><span>Scanner pair already USDT/USDC style.</span></div>
    <div><b>Binance Mirror</b><span>Original DEX pair differs; Binance opens token/USDT mirror.</span></div>
    <div><b>Chart Only</b><span>No safe CEX route. Check chart manually.</span></div>
    <div><b>Original Chart</b><span>Always keeps source scanner chart separate.</span></div>
  </section>
}

function InvestmentDiary({ investments, records, onManualAddInvestment, onCloseInvestment, onRemoveInvestment }){
  ...
  const tokenMap = useMemo(()=>{
    const m=new Map();
    (records||[]).forEach(r=>{
      const t=String(r.token||String(r.pairLabel||"").split("/")[0]).toUpperCase();
      if(!m.has(t)) m.set(t,r);
      const pair=String(r.pairLabel||"").toUpperCase();
      if(pair) m.set(pair,r);
    });
    return m;
  },[records]);

  function autofillFromLive(){
    const key = String(manual.pairLabel || manual.token || "").toUpperCase();
    const token = String(manual.token || key.split("/")[0] || "").toUpperCase();
    const r = tokenMap.get(key) || tokenMap.get(token);
    if(!r) return;
    const entry = n(r.currentPrice || r.signalPrice);
    const capital = n(manual.investedCapital,10);
    const os = orderSetupFromRecord(r, {agLabel:"MANUAL", agSize:capital}, capital);
    setManual({
      pairLabel: r.pairLabel || `${token}/USDT`,
      token: token || r.token,
      entryPrice: orderPrice(entry),
      investedCapital: String(capital),
      quantity: os.qty ? os.qty.toPrecision(7) : "",
      tp1: orderPrice(r.tp1),
      tp2: orderPrice(r.tp2),
      stopTrigger: orderPrice(os.stopTrigger),
      stopLimit: orderPrice(os.stopLimit),
      hardInvalid: orderPrice(r.invalidation)
    });
  }
  ...
  return <section className="investment-diary">
    <div className="lab-head">
      <div>
        <p className="eyebrow">Investment Diary · Binance TP/SL Tracker</p>
        <h2>Coins you already executed in Binance</h2>
        <p className="muted">If the star row changed or disappeared, add your real Binance holding manually here. Execution is still manual in Binance.</p>
      </div>
    </div>
    <BinanceTermsGuide />
    <RouteSafetyGuide />

    <section className="manual-invest-card">
      <div>
        <p className="eyebrow">Manual Add Investment</p>
        <h3>Add current Binance holding</h3>
        <p>Use this when the list changes and the ★ row is no longer available.</p>
      </div>
      <div className="manual-grid">
        <label>Pair<input value={manual.pairLabel} onChange={e=>setManual({...manual,pairLabel:e.target.value,token:String(e.target.value).split("/")[0]})} placeholder="ETH/USDT"/></label>
        <label>Token<input value={manual.token} onChange={e=>setManual({...manual,token:e.target.value})} placeholder="ETH"/></label>
        <label>Entry<input value={manual.entryPrice} onChange={e=>setManual({...manual,entryPrice:e.target.value})} placeholder="2313.96"/></label>
        <label>Capital $<input value={manual.investedCapital} onChange={e=>setManual({...manual,investedCapital:e.target.value})} placeholder="10"/></label>
        <label>Qty<input value={manual.quantity} onChange={e=>setManual({...manual,quantity:e.target.value})} placeholder="optional"/></label>
        <label>TP Limit<input value={manual.tp1} onChange={e=>setManual({...manual,tp1:e.target.value})} placeholder="optional"/></label>
        <label>Reference TP2<input value={manual.tp2} onChange={e=>setManual({...manual,tp2:e.target.value})} placeholder="optional"/></label>
        <label>SL Trigger<input value={manual.stopTrigger} onChange={e=>setManual({...manual,stopTrigger:e.target.value})} placeholder="Binance SL Trigger"/></label>
        <label>SL Price<input value={manual.stopLimit} onChange={e=>setManual({...manual,stopLimit:e.target.value})} placeholder="Binance SL Price"/></label>
        <label>System Hard Invalid<input value={manual.hardInvalid} onChange={e=>setManual({...manual,hardInvalid:e.target.value})} placeholder="optional"/></label>
      </div>
      <div className="binance-term-note"><b>Binance field mapping:</b> TP Limit = take-profit sell price · SL Trigger = price that activates stop-loss · SL Price = sell limit price after trigger · Amount = coin quantity / Max.</div>
      <div className="manual-actions">
        <button className="secondary" onClick={autofillFromLive}>Autofill from live DB</button>
        <button onClick={()=>onManualAddInvestment(manual)}>Add to Diary</button>
      </div>
    </section>
      ...
      <table className="tracker-table investment-table">
        <thead>
          <tr>
            ...
            <th>TP1 / TP2</th>
            <th>Binance TP/SL Fields</th>
            <th>System Hard Invalid</th>
            <th>Route</th>
            <th>Diary Action</th>
          </tr>
        </thead>
        <tbody>
          {rows.length===0 ? <tr><td colSpan="11" className="empty-cell">No investment diary yet. Use Manual Add or mark ★ from Trade Action Center after you execute in Binance.</td></tr> : rows.map(inv => (
            <tr key={inv.diaryId}>
              <td><span className={`aggressive-pill ${inv.sig.cls}`}>{inv.sig.label}</span><small>{inv.sig.reason}</small></td>
              <td>{(()=>{const rq=routeForAnyHolding(inv.pairLabel, inv.token, inv.url); return <><a href={inv.url || rq.routeUrl} target="_blank" rel="noreferrer">{inv.pairLabel}</a><small>{inv.actionLabel} · Rank {inv.priorityScore}</small><small>{rq.routeMode} → {rq.routePair}</small></>})()}</td>
              ...
              <td>${price(inv.tp1)} / ${price(inv.tp2)}<small>{inv.tpText || `TP1: ${price(inv.tp1)} | TP2: ${price(inv.tp2)}`}</small></td>
              <td>
                <div className="binance-fields">
                  <b>TP Limit {orderPrice(inv.tp1)}</b>
                  <small>SL Trigger {orderPrice(inv.stopTrigger || inv.entryPrice*0.98)}</small>
                  <small>SL Price {orderPrice(inv.stopLimit || (inv.entryPrice*0.978))}</small>
                  <small>Amount MAX / {n(inv.quantity).toPrecision(6)} {inv.token}</small>
                </div>
                <button className="mini-btn" onClick={()=>copyText(`BINANCE TP/SL ${inv.pairLabel}\nTP Limit: ${orderPrice(inv.tp1)}\nSL Trigger: ${orderPrice(inv.stopTrigger || inv.entryPrice*0.98)}\nSL Price: ${orderPrice(inv.stopLimit || inv.entryPrice*0.978)}\nAmount: MAX / ${n(inv.quantity).toPrecision(6)} ${inv.token}\nReference TP2: ${orderPrice(inv.tp2)}\nHard Invalid: ${orderPrice(inv.stopPrice)}`)}>Copy Binance fields</button>
              </td>
              <td>${price(inv.stopPrice)}<small>System invalidation, not practical stop.</small></td>
              <td>{(()=>{const rq=routeForAnyHolding(inv.pairLabel, inv.token, inv.url); return <><a className="action-link" href={rq.routeUrl} target="_blank" rel="noreferrer">{rq.routeMode === "Binance Mirror" ? `Binance ${rq.routePair} Mirror` : rq.routeMode === "Binance Direct" ? `Binance ${rq.routePair}` : "Open Chart"}</a><small className={rq.routeMode==="Binance Mirror"?"route-warn":""}>{rq.routeMode} {rq.routeWarning ? "· " + rq.routeWarning : ""}</small>{inv.url && <a className="action-link ghost" href={inv.url} target="_blank" rel="noreferrer">Original Chart</a>}</>})()}</td>
              <td>
...
function Lab({records,remoteLoaded,onExport,onImport,onClear,onSync}){
  const cleanRecords = useMemo(()=>usefulLabRecords(records),[records]);
  const [f,setF]=useState("ACTIVE"),[cat,setCat]=useState("ALL"),[sec,setSec]=useState("ALL"),[sort,setSort]=useState("lastUpdated"),[dir,setDir]=useState("desc");
  function click(k){if(sort===k)setDir(dir==="asc"?"desc":"asc");else{setSort(k);setDir("desc")}}
  const filtered=useMemo(()=>{let a=f==="ALL"?cleanRecords:f==="ACTIVE"?cleanRecords.filter(isActiveTradeValid):f==="SUCCESS"?cleanRecords.filter(r=>r.isSuccessArchive||isRecordSuccess(r)):cleanRecords.filter(r=>r.signalType===f); a=cat==="ALL"?a:a.filter(r=>(r.category||"").toUpperCase()===cat); a=sec==="ALL"?a:a.filter(r=>(r.securityStatus||"").toUpperCase()===sec); return[...a].sort((x,y)=>{let av=x[sort],bv=y[sort];if(String(sort).toLowerCase().includes("updated")||String(sort).toLowerCase().includes("at")){av=new Date(av||0).getTime();bv=new Date(bv||0).getTime()}else{av=n(av,av);bv=n(bv,bv)}return dir==="asc"?(av>bv?1:av<bv?-1:0):(av<bv?1:av>bv?-1:0)})},[cleanRecords,f,cat,sec,sort,dir]);
  const sum=useMemo(()=>{const records=cleanRecords; const t=records.length;return{t,b:records.filter(r=>r.signalType==="BUY").length,s:records.filter(r=>r.signalType==="SELL").length,p:records.filter(r=>r.category==="PRIME WATCH").length,g:records.filter(r=>r.category==="GOLDEN WATCH").length,np:records.filter(r=>r.category==="NEW PAIR").length,sec:t?records.reduce((a,r)=>a+n(r.securityScore),0)/t:0,z:records.filter(r=>r.hitZone).length,tr:records.filter(r=>r.hitTrigger).length,tp:records.filter(r=>r.hitTP1).length,iv:records.filter(r=>r.hitInvalidation).length,fg:t?records.reduce((a,r)=>a+n(r.forecastGoodPct),0)/t:0,ad:t?records.reduce((a,r)=>a+n(r.adverseMovePct),0)/t:0}},[cleanRecords]);
  const TH=({k,children})=><th className="sortable" onClick={()=>click(k)}>{children} {sort===k?(dir==="asc"?"↑":"↓"):""}</th>;
  return <section className="performance-lab"><div className="lab-head"><div><p className="eyebrow">Performance Lab · Fresh Active + Success Only</p><h2>Clean database: active valid setups + success records</h2><p className="muted">Invalid, stale, past-target and failed records are hidden/pruned. Only useful active setups and success records remain.</p></div><div className="lab-actions"><button className="secondary" onClick={onSync}>Sync Remote DB</button><button className="secondary" onClick={onExport}>Export JSON</button><label className="import-btn">Import JSON<input type="file" accept="application/json" onChange={onImport}/></label><button className="danger-btn" onClick={onClear}>Clear Local</button></div></div><div className="lab-note"><b>Remote loaded:</b> {remoteLoaded?"YES":"NO"} · Invalid/past-target records are automatically removed from active views.</div><section className="stats lab-stats"><div><span>Total</span><b>{sum.t}</b></div><div><span>Buy</span><b>{sum.b}</b></div><div><span>Sell</span><b>{sum.s}</b></div><div><span>Prime</span><b>{sum.p}</b></div><div><span>Golden</span><b>{sum.g}</b></div><div><span>New Pair</span><b>{sum.np}</b></div><div><span>Avg Security</span><b>{sum.sec.toFixed(0)}</b></div><div><span>Zone Hit</span><b>{sum.z}</b></div><div><span>Trigger</span><b>{sum.tr}</b></div><div><span>TP1</span><b>{sum.tp}</b></div><div><span>Invalid</span><b>{sum.iv}</b></div><div><span>Forecast Move</span><b>{sum.fg.toFixed(2)}%</b></div><div><span>Adverse</span><b>{sum.ad.toFixed(2)}%</b></div></section><div className="lab-filters"><select value={f} onChange={e=>setF(e.target.value)}><option value="ACTIVE">Active Valid Only</option><option value="SUCCESS">Success Records</option><option value="ALL">Valid + Success</option><option value="BUY">Buy Signals</option><option value="SELL">Sell Signals</option></select><select value={cat} onChange={e=>setCat(e.target.value)}><option value="ALL">All Categories</option><option value="BUY WATCH">Buy Watch</option><option value="PRIME WATCH">Prime Watch</option><option value="SELL WATCH">Sell Watch</option><option value="GOLDEN WATCH">Golden Watch</option><option value="NEW PAIR">New Pair</option></select><select value={sec} onChange={e=>setSec(e.target.value)}><option value="ALL">All Security</option><option value="PASS">Pass</option><option value="CAUTION">Caution</option><option value="HIGH RISK">High Risk</option><option value="BLOCKED">Blocked</option></select><select value={dir} onChange={e=>setDir(e.target.value)}><option value="desc">Highest / Latest first</option><option value="asc">Lowest / Oldest first</option></select></div><div className="table-wrap"><table className="tracker-table"><thead><tr><TH k="trackedAt">Age</TH><TH k="signalType">Signal</TH><TH k="category">Category</TH><TH k="securityScore">Security</TH><th>Pair</th><TH k="status">Status</TH><TH k="signalPrice">Signal Price</TH><TH k="currentPrice">Current</TH><TH k="currentMovePct">Move</TH><TH k="forecastGoodPct">Forecast</TH><TH k="adverseMovePct">Adverse</TH><TH k="zoneLow">Zone</TH><TH k="trigger">Trigger</TH><TH k="tp1">TP1/TP2</TH><TH k="invalidation">Invalid</TH><th>Hits</th><TH k="lastUpdated">Updated</TH></tr></thead><tbody>{filtered.length===0?<tr><td colSpan="17" className="empty-cell">No records.</td></tr>:filtered.map(r=><tr key={r.id}><td>{age(r.trackedAt)}</td><td><span className={`signal-pill ${r.signalType==="SELL"?"sell":"buy"}`}>{r.signalType}</span></td><td><span className={`category-pill ${String(r.category).includes("GOLDEN")?"golden":String(r.category).includes("PRIME")?"prime":String(r.category).includes("NEW")?"newpair":r.signalType==="SELL"?"sell":"buy"}`}>{r.category||r.signal}</span></td><td><span className={`security-pill ${String(r.securityStatus).includes("PASS")?"pass":String(r.securityStatus).includes("CAUTION")?"caution":String(r.securityStatus).includes("BLOCKED")?"blocked":"risk"}`}>{r.securityStatus||"UNKNOWN"} {n(r.securityScore).toFixed(0)}</span></td><td><a href={r.url} target="_blank" rel="noreferrer">{r.pairLabel}</a><small>{r.chain} · {r.dex}</small></td><td><span className={`status-pill ${String(r.result).includes("WIN")||r.isSuccessArchive?"win":String(r.result).includes("FAILED")?"fail":"active"}`}>{r.isSuccessArchive ? r.resultStatus : r.status}</span></td><td>${price(r.signalPrice)}</td><td>${price(r.currentPrice)}</td><td className={n(r.currentMovePct)>=0?"up":"down"}>{n(r.currentMovePct).toFixed(2)}%</td><td className="up">{n(r.forecastGoodPct).toFixed(2)}%</td><td className="down">{n(r.adverseMovePct).toFixed(2)}%</td><td>${price(r.zoneLow)}–${price(r.zoneHigh)}</td><td>${price(r.trigger)}</td><td>${price(r.tp1)} / ${price(r.tp2)}</td><td>${price(r.invalidation)}</td><td><div className="hit-grid"><span className={r.hitZone?"hit yes":"hit"}>ZN</span><span className={r.hitTrigger?"hit yes":"hit"}>TR</span><span className={r.hitTP1?"hit yes":"hit"}>T1</span><span className={r.hitTP2?"hit yes":"hit"}>T2</span><span className={r.hitInvalidation?"hit bad":"hit"}>INV</span></div></td><td>{new Date(r.lastUpdated||r.trackedAt).toLocaleString()}</td></tr>)}</tbody></table></div></section>
}

export default 
function CustomizeInvestmentPlan({ records, investments, hunterMode }){
  const [capital,setCapital]=useState(50);
  const [mode,setMode]=useState(hunterMode || 'EXTREME');
  const [maxPos,setMaxPos]=useState(4);
  const rows=useMemo(()=>{
    const mapped=validActionRecords(records).map(r=>{const advisory=routeAdvisor(r,capital,mode); advisory.priorityScore=tradePriorityScore(r,advisory,capital); return {...r,advisory};})
      .filter(r=>n(r.advisory.agSize)>0 && !String(r.advisory.agLabel).includes('SKIP') && !String(r.advisory.agLabel).includes('TOO LATE') && !String(r.advisory.agLabel).includes('WARNING'));
    const best=new Map();
    for(const r of mapped){const key=r.advisory.routePair||r.pairLabel; const old=best.get(key); if(!old||n(r.advisory.priorityScore)>n(old.advisory.priorityScore)) best.set(key,r);}
    return [...best.values()].sort((a,b)=>n(b.advisory.priorityScore)-n(a.advisory.priorityScore)).slice(0,n(maxPos,4));
  },[records,capital,mode,maxPos]);
  const deploy=rows.reduce((s,r)=>s+n(r.advisory.agSize),0);
  const reserve=Math.max(0,n(capital)-deploy);
  const tp1=rows.reduce((s,r)=>s+n(r.advisory.tp1Profit),0);
  const tp2=rows.reduce((s,r)=>s+n(r.advisory.tp2Profit),0);
  const risk=rows.reduce((s,r)=>s+n(r.advisory.invalidLoss),0);
  function planText(){return [`SNITCH X CUSTOMIZE INVESTMENT PLAN`,`Capital: $${n(capital).toFixed(2)}`,`Mode: ${mode}`,`Deploy Now: $${deploy.toFixed(2)}`,`Reserve: $${reserve.toFixed(2)}`,``,...rows.map((r,i)=>{const a=r.advisory, os=orderSetupFromRecord(r,a,capital); return `${i+1}. ${r.pairLabel} — ${a.agLabel} — $${n(a.agSize).toFixed(2)}\nBucket: ${a.bucket}\nRoute: ${a.routeMode||a.route} → ${a.routePair||r.pairLabel}\nTP Limit: ${orderPrice(os.tp1)}\nSL Trigger: ${orderPrice(os.stopTrigger)}\nSL Price: ${orderPrice(os.stopLimit)}\nAmount: ${os.qty.toPrecision(6)} ${r.token}\nReason: ${a.agReason}`;})].join('\n');}
  return <section className="trade-center plan-builder"><div className="lab-head"><div><p className="eyebrow">Customize Investment Plan · v10.15</p><h2>Capital attack plan</h2><p className="muted">Builds a deployment plan from fresh valid records only. Success Forecast and Investment Diary remain the only historical storage.</p></div><div className="capital-box"><span>Total capital</span><div className="capital-input-row"><b>$</b><input type="number" min="1" step="1" value={capital} onChange={e=>setCapital(e.target.value)} /></div></div></div><div className="lab-filters"><select value={mode} onChange={e=>setMode(e.target.value)}><option value="BALANCED">Balanced</option><option value="AGGRESSIVE">Aggressive</option><option value="EXTREME">Extreme Hunter</option></select><select value={maxPos} onChange={e=>setMaxPos(e.target.value)}><option value="2">Max 2 positions</option><option value="3">Max 3 positions</option><option value="4">Max 4 positions</option><option value="6">Max 6 positions</option></select><button className="secondary" onClick={()=>copyText(planText())}>Copy Plan</button></div><section className="stats lab-stats"><div><span>Deploy Now</span><b>${deploy.toFixed(2)}</b></div><div><span>Reserve</span><b>${reserve.toFixed(2)}</b></div><div><span>Positions</span><b>{rows.length}</b></div><div><span>TP1 Potential</span><b>${tp1.toFixed(2)}</b></div><div><span>TP2 Potential</span><b>${tp2.toFixed(2)}</b></div><div><span>Risk If Invalid</span><b>${risk.toFixed(2)}</b></div><div><span>Diary Active</span><b>{investments.filter(x=>x.status!=="CLOSED").length}</b></div></section><div className="table-wrap"><table className="tracker-table action-table"><thead><tr><th>Priority</th><th>Action</th><th>Pair</th><th>Bucket</th><th>Size</th><th>TP/SL Binance Fields</th><th>Route</th><th>Reason</th></tr></thead><tbody>{rows.length===0?<tr><td colSpan="8" className="empty-cell">No fresh valid plan now. Wait for next sync.</td></tr>:rows.map((r,i)=>{const a=r.advisory, os=orderSetupFromRecord(r,a,capital); return <tr key={`${r.id}-plan`}><td><b>#{i+1}</b><small>Rank {a.priorityScore}</small></td><td><span className={`aggressive-pill ${String(a.agLabel).includes('GRAB')?'grab':String(a.agLabel).includes('ATTACK')?'enter':'scout'}`}>{a.agLabel}</span></td><td><a href={r.url} target="_blank" rel="noreferrer">{r.pairLabel}</a><small>{r.chain} · {r.dex}</small></td><td><span className="bucket-pill">{a.bucket}</span></td><td><b>${n(a.agSize).toFixed(2)}</b><small>of ${n(capital).toFixed(0)}</small></td><td><div className="binance-fields"><b>TP Limit {orderPrice(os.tp1)}</b><small>SL Trigger {orderPrice(os.stopTrigger)}</small><small>SL Price {orderPrice(os.stopLimit)}</small><small>Amount {os.qty.toPrecision(6)} {r.token}</small></div></td><td><a className="action-link" href={a.routeUrl} target="_blank" rel="noreferrer">{a.actionLabel}</a><small>{a.routeMode} → {a.routePair}</small></td><td>{a.agReason}</td></tr>})}</tbody></table></div></section>
}

function App(){
  const [query,setQuery]=useState(""),[items,setItems]=useState([]),[selected,setSelected]=useState(null),[status,setStatus]=useState("Ready"),[loading,setLoading]=useState(false),[tab,setTab]=useState("scanner"),[hunterMode,setHunterMode]=useState("EXTREME"),[records,setRecords]=useState([]),[investments,setInvestments]=useState([]),[remote,setRemote]=useState(false),[syncing,setSyncing]=useState(false),[lastSynced,setLastSynced]=useState(null),[syncError,setSyncError]=useState(""),[minLiquidity,setMinLiquidity]=useState(200000),[strictness,setStrictness]=useState("balanced"),[securityMode,setSecurityMode]=useState("balanced"),[showRejected,setShowRejected]=useState(false);
  const ref=useRef([]); const opts={minLiquidity,strictness,securityMode,hideRejected:!showRejected};
  useEffect(()=>{

    try{
      const migrated = localStorage.getItem("snitch_x_v10_13_migrated");
      if(!migrated){
        ["snitch_x_v6_buy_sell_lab","snitch_x_v10_12_valid_list_cleanup","snitch_x_v10_12_1_worker_async_patch"].forEach(k=>localStorage.removeItem(k));
        localStorage.setItem("snitch_x_v10_13_migrated","yes");
      }
    }catch{}
    try{
      ...
      if(Array.isArray(arr)){
        const cleaned = pruneRecordsForStorage(arr);
        ref.current=cleaned;
        setRecords(cleaned);
        localStorage.setItem(STORE,JSON.stringify(cleaned));
        const scanner=scannerItemsFromRecords(cleaned);
        setItems(scanner);
    ...
    try{
      ...
      if(Array.isArray(arrInv)) {
        const upgraded = arrInv.map(inv => {
          if(inv.stopTrigger && inv.stopLimit) return inv;
          const fallback = {
            currentPrice: inv.entryPrice,
            signalPrice: inv.entryPrice,
            invalidation: inv.stopPrice,
            tp1: inv.tp1,
            tp2: inv.tp2
          };
          const setup = orderSetupFromRecord(fallback, {agLabel: inv.actionLabel || "GRAB NOW", agSize: inv.investedCapital || 10}, inv.investedCapital || 10);
          return {
            ...inv,
            stopTrigger: setup.stopTrigger,
            stopLimit: setup.stopLimit,
        ...
        setInvestments(upgraded);
        localStorage.setItem(INVEST_STORE, JSON.stringify(upgraded));
      }
    }catch{}
  ...
  function persist(next){
    const s=pruneRecordsForStorage(next).sort((a,b)=>new Date(b.lastUpdated||b.completedAt||0)-new Date(a.lastUpdated||a.completedAt||0));
    ref.current=s;
    setRecords(s);
  ...
  async function syncRemote({updateScanner=false,silent=false}={}){
    ...
    try{
      ...
      const successRows = Array.isArray(d.successForecasts) ? d.successForecasts.map(successForecastToRecord) : [];
      const arr=Array.isArray(d.records)?[...d.records,...successRows]:Array.isArray(d)?d:[];
      ...
      if(updateScanner){
        ...
        setStatus(`Fresh sync: ${scanner.length} valid active rows only · ${new Date().toLocaleTimeString()}`);
      }
      setRemote(true);
  ...
  function addManualInvestment(form){
    const token = String(form.token || String(form.pairLabel||"").split("/")[0] || "").toUpperCase();
    const pairLabel = String(form.pairLabel || `${token}/USDT`).toUpperCase();
    const live = records.find(r => String(r.pairLabel||"").toUpperCase() === pairLabel)
      || records.find(r => String(r.token||"").toUpperCase() === token);
    const entry = n(form.entryPrice, n(live?.currentPrice || live?.signalPrice));
    const capital = n(form.investedCapital, 10);
    const qty = n(form.quantity, entry>0 ? capital/entry : 0);
    const tempRecord = {
      ...(live || {}),
      id: live?.id || `manual-${pairLabel}-${Date.now()}`,
      pairLabel,
      token,
      currentPrice: entry,
      signalPrice: entry,
      tp1: n(form.tp1, n(live?.tp1, entry*1.025)),
      tp2: n(form.tp2, n(live?.tp2, entry*1.06)),
      invalidation: n(form.hardInvalid, n(live?.invalidation, entry*0.90)),
      url: live?.url || platformTradeUrl("Binance", token),
      chain: live?.chain || "manual",
      dex: live?.dex || "binance",
      signalType: live?.signalType || "BUY",
      category: live?.category || "MANUAL",
      securityStatus: live?.securityStatus || "MANUAL",
      securityScore: live?.securityScore || 100
    };
    const setup = orderSetupFromRecord(tempRecord, {agLabel:"MANUAL", agSize:capital}, capital);
    const now = new Date().toISOString();
    const item = {
      ...investmentFromRecord(tempRecord, {agLabel:"MANUAL", agSize:capital, priorityScore:0}, capital),
      quantity: qty,
      stopTrigger: n(form.stopTrigger, setup.stopTrigger),
      stopLimit: n(form.stopLimit, setup.stopLimit),
      stopPrice: n(form.hardInvalid, setup.hardInvalid),
      tp1: n(form.tp1, setup.tp1),
      tp2: n(form.tp2, setup.tp2),
      ...
      createdAt: now,
      updatedAt: now,
      notes: "Manually added from Binance holding. Route uses Direct/Mirror/Chart logic."
    };
    saveInvestments([item, ...investments]);
    setStatus(`Manual added ${pairLabel} into Investment Diary`);
    setTab("investment");
  }
  ...
  function exp(){const blob=new Blob([JSON.stringify({version:"v10.15-full-extreme-integration",exportedAt:new Date().toISOString(),records:ref.current},null,2)],{type:"application/json"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="snitch-x-v10-15-full-extreme-integration.json";a.click();URL.revokeObjectURL(url)}
  ...
  return <main><header className="hero"><div><p className="eyebrow">SNITCH X · Full Extreme Integration v10.15</p><h1>All discussed modules reincorporated on stable UI.</h1><p className="sub">V10.15 re-adds Extreme Hunter mode, risk buckets, aggressive labels, capital plan builder, fresh-list cleanup, Success Forecast and Investment Diary.</p></div><div className="hero-card sync-card">
      <span>Status</span>
