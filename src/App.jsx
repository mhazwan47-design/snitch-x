...
function recFrom(item,source="frontend"){
  ...
  return {id:item.id,pairKey:item.pairKey,token:item.token,pairLabel:item.pairLabel,chain:item.chain,dex:item.dex,url:item.url,source,version:"v6-mixed-buy-sell",
    signalType:item.signalType,category:item.category,signal:item.decision,trackedAt:t,firstSeenAt:t,lastUpdated:t,signalPrice:item.priceUsd,currentPrice:item.priceUsd,maxHigh:item.priceUsd,maxLow:item.priceUsd,
...
function Card({item,selected,onSelect}){return <button className={`pair-card ${selected?"selected":""}`} onClick={()=>onSelect(item)}><div className="pair-top"><div><h3>{item.token}</h3><p>{item.pairLabel} · {item.chain} · {item.dex}</p></div><span className={`decision ${dclass(item)}`}>{item.decision}</span></div><div className="pair-price"><b>${price(item.priceUsd)}</b><span className={item.h24>=0?"up":"down"}>24H {item.h24.toFixed(2)}%</span></div><div className="score-grid"><Score label="Opp" value={item.opportunity}/><Score label="Exec" value={item.execution}/><Score label="Valid" value={item.validity}/><Score label="Risk" value={item.risk} inverse/></div><div className="mini-grid"><div><span>Liquidity</span><b>{money(item.liquidityUsd)}</b></div><div><span>Volume</span><b>{money(item.volume24h)}</b></div><div><span>Signal</span><b>{item.signalType}</b></div><div><span>Category</span><b>{item.category}</b></div></div></button>}
function Detail({item}){if(!item)return <section className="panel sticky-panel"><h2>Action Detail</h2><p className="muted">Run radar or search token.</p></section>;const l=item.levels, sell=item.signalType==="SELL";return <section className="panel sticky-panel"><div className="detail-head"><div><h2>{item.token} Execution Plan</h2><p>{item.pairLabel} · {item.chain} · {item.dex}</p></div><span className={`decision big ${dclass(item)}`}>{item.decision}</span></div><div className="answer-box"><span>Signal</span><b>{item.signalType} · {item.category}</b><p>{item.action}</p></div><div className="detail-grid"><div><span>Mode</span><b>{sell?"Exit / Sell Watch":"Buy Watch"}</b></div><div><span>Current</span><b>${price(item.priceUsd)}</b></div><div><span>{sell?"Sell Zone":"Buy Zone"}</span><b>${price(l.zoneLow)} – ${price(l.zoneHigh)}</b></div><div><span>{sell?"Breakdown":"Breakout"}</span><b>${price(l.trigger)}</b></div><div><span>Invalidation</span><b>${price(l.invalidation)}</b></div><div><span>{sell?"Down TP1 / TP2":"TP1 / TP2"}</span><b>${price(l.tp1)} / ${price(l.tp2)}</b></div><div><span>RR</span><b>1:{n(l.rr).toFixed(2)}</b></div><div><span>Bear Score</span><b>{Math.round(n(item.bearScore))}</b></div></div><div className="rule-box"><h4>Forecast Plan</h4><p><b>{item.decision}:</b> {sell?"Bearish pressure detected. If holding, protect capital; if not holding, avoid fresh buy.":"Bullish candidate. No market buy unless buy-zone bounce or breakout retest."}</p><p><b>Performance Lab:</b> Tracks forecast direction, trigger hit, TP hit, invalidation, adverse move and actual follow-through.</p></div><a className="dex-link" href={item.url} target="_blank" rel="noreferrer">Open live chart on DexScreener</a></section>}

function Lab({records,remoteLoaded,onExport,onImport,onClear,onSync}){
  const [f,setF]=useState("ALL"),[cat,setCat]=useState("ALL"),[sort,setSort]=useState("lastUpdated"),[dir,setDir]=useState("desc");
  function click(k){if(sort===k)setDir(dir==="asc"?"desc":"asc");else{setSort(k);setDir("desc")}}
  const filtered=useMemo(()=>{let a=f==="ALL"?records:records.filter(r=>r.signalType===f); a=cat==="ALL"?a:a.filter(r=>(r.category||"").toUpperCase()===cat); return[...a].sort((x,y)=>{let av=x[sort],bv=y[sort];if(String(sort).toLowerCase().includes("updated")||String(sort).toLowerCase().includes("at")){av=new Date(av||0).getTime();bv=new Date(bv||0).getTime()}else{av=n(av,av);bv=n(bv,bv)}return dir==="asc"?(av>bv?1:av<bv?-1:0):(av<bv?1:av>bv?-1:0)})},[records,f,cat,sort,dir]);
  const sum=useMemo(()=>{const t=records.length;return{t,b:records.filter(r=>r.signalType==="BUY").length,s:records.filter(r=>r.signalType==="SELL").length,g:records.filter(r=>r.category==="GOLDEN WATCH").length,np:records.filter(r=>r.category==="NEW PAIR").length,z:records.filter(r=>r.hitZone).length,tr:records.filter(r=>r.hitTrigger).length,tp:records.filter(r=>r.hitTP1).length,iv:records.filter(r=>r.hitInvalidation).length,fg:t?records.reduce((a,r)=>a+n(r.forecastGoodPct),0)/t:0,ad:t?records.reduce((a,r)=>a+n(r.adverseMovePct),0)/t:0}},[records]);
  const TH=({k,children})=><th className="sortable" onClick={()=>click(k)}>{children} {sort===k?(dir==="asc"?"↑":"↓"):""}</th>;
  return <section className="performance-lab"><div className="lab-head"><div><p className="eyebrow">Performance Lab · Mixed Buy/Sell</p><h2>Forecast vs actual database</h2><p className="muted">One table. Signal column shows BUY or SELL. Category tracks BUY WATCH, SELL WATCH, GOLDEN WATCH and NEW PAIR.</p></div><div className="lab-actions"><button className="secondary" onClick={onSync}>Sync Remote DB</button><button className="secondary" onClick={onExport}>Export JSON</button><label className="import-btn">Import JSON<input type="file" accept="application/json" onChange={onImport}/></label><button className="danger-btn" onClick={onClear}>Clear Local</button></div></div><div className="lab-note"><b>Remote loaded:</b> {remoteLoaded?"YES":"NO"} · Click table headers to sort highest/lowest.</div><section className="stats lab-stats"><div><span>Total</span><b>{sum.t}</b></div><div><span>Buy</span><b>{sum.b}</b></div><div><span>Sell</span><b>{sum.s}</b></div><div><span>Golden</span><b>{sum.g}</b></div><div><span>New Pair</span><b>{sum.np}</b></div><div><span>Zone Hit</span><b>{sum.z}</b></div><div><span>Trigger</span><b>{sum.tr}</b></div><div><span>TP1</span><b>{sum.tp}</b></div><div><span>Invalid</span><b>{sum.iv}</b></div><div><span>Forecast Move</span><b>{sum.fg.toFixed(2)}%</b></div><div><span>Adverse</span><b>{sum.ad.toFixed(2)}%</b></div></section><div className="lab-filters"><select value={f} onChange={e=>setF(e.target.value)}><option value="ALL">All Signals</option><option value="BUY">Buy Signals</option><option value="SELL">Sell Signals</option></select><select value={cat} onChange={e=>setCat(e.target.value)}><option value="ALL">All Categories</option><option value="BUY WATCH">Buy Watch</option><option value="SELL WATCH">Sell Watch</option><option value="GOLDEN WATCH">Golden Watch</option><option value="NEW PAIR">New Pair</option></select><select value={dir} onChange={e=>setDir(e.target.value)}><option value="desc">Highest / Latest first</option><option value="asc">Lowest / Oldest first</option></select></div><div className="table-wrap"><table className="tracker-table"><thead><tr><TH k="trackedAt">Age</TH><TH k="signalType">Signal</TH><TH k="category">Category</TH><th>Pair</th><TH k="status">Status</TH><TH k="signalPrice">Signal Price</TH><TH k="currentPrice">Current</TH><TH k="currentMovePct">Move</TH><TH k="forecastGoodPct">Forecast</TH><TH k="adverseMovePct">Adverse</TH><TH k="zoneLow">Zone</TH><TH k="trigger">Trigger</TH><TH k="tp1">TP1/TP2</TH><TH k="invalidation">Invalid</TH><th>Hits</th><TH k="lastUpdated">Updated</TH></tr></thead><tbody>{filtered.length===0?<tr><td colSpan="16" className="empty-cell">No records.</td></tr>:filtered.map(r=><tr key={r.id}><td>{age(r.trackedAt)}</td><td><span className={`signal-pill ${r.signalType==="SELL"?"sell":"buy"}`}>{r.signalType}</span></td><td><span className={`category-pill ${String(r.category).includes("GOLDEN")?"golden":String(r.category).includes("NEW")?"newpair":r.signalType==="SELL"?"sell":"buy"}`}>{r.category||r.signal}</span></td><td><a href={r.url} target="_blank" rel="noreferrer">{r.pairLabel}</a><small>{r.chain} · {r.dex}</small></td><td><span className={`status-pill ${String(r.result).includes("WIN")?"win":String(r.result).includes("FAILED")?"fail":"active"}`}>{r.status}</span></td><td>${price(r.signalPrice)}</td><td>${price(r.currentPrice)}</td><td className={n(r.currentMovePct)>=0?"up":"down"}>{n(r.currentMovePct).toFixed(2)}%</td><td className="up">{n(r.forecastGoodPct).toFixed(2)}%</td><td className="down">{n(r.adverseMovePct).toFixed(2)}%</td><td>${price(r.zoneLow)}–${price(r.zoneHigh)}</td><td>${price(r.trigger)}</td><td>${price(r.tp1)} / ${price(r.tp2)}</td><td>${price(r.invalidation)}</td><td><div className="hit-grid"><span className={r.hitZone?"hit yes":"hit"}>ZN</span><span className={r.hitTrigger?"hit yes":"hit"}>TR</span><span className={r.hitTP1?"hit yes":"hit"}>T1</span><span className={r.hitTP2?"hit yes":"hit"}>T2</span><span className={r.hitInvalidation?"hit bad":"hit"}>INV</span></div></td><td>{new Date(r.lastUpdated||r.trackedAt).toLocaleString()}</td></tr>)}</tbody></table></div></section>
}

export default function App(){
  const [query,setQuery]=useState(""),[items,setItems]=useState([]),[selected,setSelected]=useState(null),[status,setStatus]=useState("Ready"),[loading,setLoading]=useState(false),[tab,setTab]=useState("scanner"),[records,setRecords]=useState([]),[remote,setRemote]=useState(false),[minLiquidity,setMinLiquidity]=useState(200000),[strictness,setStrictness]=useState("balanced"),[showRejected,setShowRejected]=useState(false);
  const ref=useRef([]); const opts={minLiquidity,strictness,hideRejected:!showRejected};
  ...
  function process(pairs,q){const a=(pairs||[]).map(p=>{const x=analyze(p,q,opts); x.category=getCategory(x); return x}).filter(x=>showRejected?true:x.decision!=="FILTERED");const m=new Map();a.forEach(x=>{const key=`${String(x.token).toUpperCase()}-${x.signalType}`;const e=m.get(key);if(!e||rank(x)>rank(e))m.set(key,x)});return[...m.values()].sort((a,b)=>rank(b)-rank(a)).slice(0,50)}
