...
function platformSearchUrl(platform, token){
  const t = encodeURIComponent(String(token || "").toUpperCase());
  if(platform === "Binance") return `https://www.binance.com/en/search?query=${t}`;
  if(platform === "OKX") return `https://www.okx.com/search?keyword=${t}`;
  if(platform === "Bybit") return `https://www.bybit.com/en/search?keyword=${t}`;
  if(platform === "Kraken") return `https://www.kraken.com/search?query=${t}`;
...
function routeAdvisor(record, capital){
  ...
  if(secStatus === "BLOCKED" || secStatus === "HIGH RISK"){
    ...
    route = "CEX Preferred";
    platform = "Binance";
    actionLabel = "Open Binance Search";
    routeUrl = platformSearchUrl("Binance", token);
    ...
    actionLabel = listedStyle ? "Open Binance Search" : "Open Chart";
    routeUrl = listedStyle ? platformSearchUrl("Binance", token) : (record.url || "#");
  ...
  return { ...pnl, route, platform, actionLabel, routeUrl, viability, reason, feeTier:fee.feeTier };
}
...
function TradeActionCenter({ records }){
  ...
  const rows = useMemo(() => {
    ...
    if(filter === "ACTIONABLE") filtered = base.filter(r => !String(r.advisory.viability).includes("AVOID") && !String(r.advisory.viability).includes("NOT WORTH"));
  ...
  return (
    <section className="trade-center">
      <div className="lab-head">
        <div>
          <p className="eyebrow">Trade Action Center · v9</p>
...
export default function App(){
  ...
  return <main><header className="hero"><div><p className="eyebrow">SNITCH X · Trade Action Center v9</p><h1>Calculate profit, route and action in one cockpit.</h1><p className="sub">V9 adds capital calculator, route advisor, action buttons and shareable trade plans.</p></div><div className="hero-card"><span>Status</span><b>{loading?"Scanning...":status}</b></div></header><section className="tabbar"><button className={tab==="scanner"?"tab active":"tab"} onClick={()=>setTab("scanner")}>Scanner</button><button className={tab==="lab"?"tab active":"tab"} onClick={()=>setTab("lab")}>Performance Lab</button><button className={tab==="action"?"tab active":"tab"} onClick={()=>setTab("action")}>Trade Action Center</button></section>{tab==="scanner"?<><section className="toolbar"><input value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>e.key==="Enter"&&search()} placeholder="Search token e.g. ETH, PEPE"/><select value={minLiquidity} onChange={e=>setMinLiquidity(Number(e.target.value))}><option value="0">No liquidity filter</option><option value="50000">Min liquidity $50K</option><option value="200000">Min liquidity $200K</option><option value="1000000">Min liquidity $1M</option></select><select value={strictness} onChange={e=>setStrictness(e.target.value)}><option value="hunter">hunter</option><option value="balanced">balanced</option><option value="strict">strict</option></select><select value={securityMode} onChange={e=>setSecurityMode(e.target.value)}><option value="loose">security loose</option><option value="balanced">security balanced</option><option value="strict">security strict</option></select><button onClick={()=>search()} disabled={loading}>Search</button><button className="secondary" onClick={radar} disabled={loading}>Run Radar</button></section><section className="toggle-row"><label><input type="checkbox" checked={showRejected} onChange={e=>setShowRejected(e.target.checked)}/>Show rejected / bad-data pairs</label></section><section className="stats"><div><span>Pairs</span><b>{items.length}</b></div><div><span>Buy Watch</span><b>{stats.buy}</b></div><div><span>Sell Watch</span><b>{stats.sell}</b></div><div><span>Tracked DB</span><b>{records.length}</b></div><div><span>Avg Execution</span><b>{stats.avg}</b></div></section><section className="layout"><div className="list">{items.length===0?<div className="empty"><h2>No signals loaded yet</h2><p>Run Radar to collect mixed BUY WATCH and SELL WATCH records.</p></div>:items.map(it=><Card key={it.id} item={it} selected={selected?.id===it.id} onSelect={setSelected}/>)}</div><Detail item={selected}/></section></>:tab==="lab"?<Lab records={records} remoteLoaded={remote} onExport={exp} onImport={imp} onClear={clr} onSync={syncRemote}/>:<TradeActionCenter records={records}/>}</main>
}
