...
function tokenRiskBucket(record){
  ...
  const newNarr = new Set(["AI","AIXBT","VIRTUAL","GRASS","KAITO","BERA","ZRO","AEVO","EIGEN","ALT","ARKM","CETUS","DEEP"]);
  if(cat.includes("NEW") || /NEW/i.test(pair)) return "NEW PAIR";
  if(highVol.has(token)) return "HIGH VOL";
  if(newNarr.has(token)) return "NARRATIVE";
  if(CEX_TOKEN_SET.has(token)) return "CEX SAFE";
  return "WILD CARD";
}
function hunterActionLabel(record, baseLabel){
  const bucket = tokenRiskBucket(record);
  const label = String(baseLabel||"");
  if(label.includes("GRAB")) return "GRAB NOW";
  if(bucket === "HIGH VOL") return "HIGH VOL SCOUT";
  if(bucket === "NEW PAIR") return "NEW PAIR HUNTER";
  if(bucket === "NARRATIVE") return "ATTACK ENTRY";
  if(label.includes("ENTER")) return "ATTACK ENTRY";
  if(label.includes("SCOUT")) return "SCOUT ENTRY";
  return label;
}
...
function sizeByAction(capital, action, bucket, mode="EXTREME"){
  const cap = n(capital,10);
  const a = String(action||"");
  let pct = 0.10;
  if(a.includes("GRAB")) pct = 0.70;
  else if(a.includes("ATTACK")) pct = 0.45;
  else if(a.includes("HIGH VOL")) pct = 0.25;
  else if(a.includes("NEW PAIR")) pct = 0.15;
  else if(a.includes("SCOUT")) pct = 0.20;
  const size = cap * pct * modeSizeMultiplier(mode,bucket);
  return Math.max(0, Math.min(cap, size));
}
...
function aggressivePriorityForRecord(record, capital=10, mode="EXTREME"){
  ...
  const rem = Math.max(tp1Pct,tp2Pct,0);
  const m = String(mode||"EXTREME").toUpperCase();

  if(isSell) return {agLabel:"SKIP / SELL WATCH", agSize:0, agScore:0, agReason:"Sell watch. No spot buy.", agTP1:tp1Pct, agTP2:tp2Pct, agRisk:riskPct, bucket};
  ...
  if(record.hitTP1 && rem < 2.5) return {agLabel:"CHASE WARNING", agSize:0, agScore:0, agReason:"Already hit TP1 and remaining room is small.", agTP1:tp1Pct, agTP2:tp2Pct, agRisk:riskPct, bucket};

  let label = "WATCH ONLY";
  let score = 0;
  let reason = "Valid but not strong enough.";
  if(sec >= 95 && record.hitTrigger && tp2Pct >= 4 && riskPct <= 14){
    label = "GRAB NOW";
    score = 95;
    reason = "Trigger confirmed, security strong, remaining TP2 still attractive.";
  } else if(sec >= 85 && (record.hitZone || record.hitTrigger) && tp2Pct >= 5 && riskPct <= 18){
    label = "ENTER SMALL";
    score = 78;
    reason = "Fresh opportunity with enough upside.";
  } else if(sec >= 70 && tp2Pct >= 8 && riskPct <= 22){
    label = "SCOUT ENTRY";
    score = 62;
    reason = "Early setup with upside. Scout only.";
  }

  // Extreme mode unlocks high-vol / new-pair aggressive attempts.
  if(m === "EXTREME"){
    if((bucket === "HIGH VOL" || bucket === "NARRATIVE") && sec >= 65 && tp2Pct >= 7 && riskPct <= 28){
      label = bucket === "HIGH VOL" ? "HIGH VOL SCOUT" : "ATTACK ENTRY";
      score = Math.max(score, 72);
      reason = bucket === "HIGH VOL" ? "High volatility setup. Small fast scout only." : "Narrative/momentum attack entry.";
    }
    if(bucket === "NEW PAIR" && sec >= 60 && tp2Pct >= 12 && riskPct <= 35){
      label = "NEW PAIR HUNTER";
      score = Math.max(score, 68);
      reason = "New pair hunter setup. Very risky; small size only.";
    }
  } else if(m === "AGGRESSIVE"){
    if((bucket === "HIGH VOL" || bucket === "NARRATIVE") && sec >= 75 && tp2Pct >= 8 && riskPct <= 24){
      label = bucket === "HIGH VOL" ? "HIGH VOL SCOUT" : "ATTACK ENTRY";
      score = Math.max(score, 68);
      reason = "Aggressive mode allows this risk bucket.";
    }
  }

  const finalLabel = hunterActionLabel(record,label);
  const size = sizeByAction(capital, finalLabel, bucket, mode);
  return {agLabel:finalLabel, agSize:Math.round(size*100)/100, agScore:score, agReason:reason, agTP1:tp1Pct, agTP2:tp2Pct, agRisk:riskPct, bucket};
}
...
function TradeActionCenter({ records, investments, onMarkInvestment, hunterMode }){
        ...
        <table className="tracker-table action-table">
          <thead>
            <tr>
              ...
              <th>Star</th>
              <th>Aggressive Action</th><th>Bucket</th>
              <th>Pair</th>
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr><td colSpan="12" className="empty-cell">No suggested trade list for this modal/filter.</td></tr>
            ) : rows.map((r) => {
              const a = r.advisory;
...
function routeForAnyHolding(pairLabel, token, originalUrl){
  const rq = routeQualityForPair({ pairLabel, token, url: originalUrl });
  return rq;
}
function routeBadgeText(routeMode){
  if(routeMode === "Binance Direct") return "Direct";
  if(routeMode === "Binance Mirror") return "Mirror";
  return "Chart";
}
...
function CustomizeInvestmentPlan({ records, investments, hunterMode }){
  ...
  const rows = useMemo(()=>{
    const mapped = validActionRecords(records).map(r=>{
      const advisory = routeAdvisor(r, capital, mode);
      advisory.priorityScore = tradePriorityScore(r, advisory, capital);
      return {...r, advisory};
    }).filter(r=>n(r.advisory.agSize)>0 && !String(r.advisory.agLabel).includes("SKIP") && !String(r.advisory.agLabel).includes("TOO LATE"));
    const byRoute = new Map();
    for(const r of mapped){
      const key = r.advisory.routePair || r.pairLabel;
      const old = byRoute.get(key);
      if(!old || n(r.advisory.priorityScore)>n(old.advisory.priorityScore)) byRoute.set(key,r);
    }
    return [...byRoute.values()].sort((a,b)=>n(b.advisory.priorityScore)-n(a.advisory.priorityScore)).slice(0,n(maxPos,4));
  ...
  function planText(){
    return [`SNITCH X CUSTOMIZE INVESTMENT PLAN`,
      `Capital: $${n(capital).toFixed(2)}`,
      `Mode: ${mode}`,
      `Deploy Now: $${deploy.toFixed(2)}`,
      `Reserve: $${reserve.toFixed(2)}`,
      ``,
      ...rows.map((r,i)=>{
        const a=r.advisory, os=orderSetupFromRecord(r,a,capital);
        return `${i+1}. ${r.pairLabel} — ${a.agLabel} — $${n(a.agSize).toFixed(2)}
Bucket: ${a.bucket}
...
Reason: ${a.agReason}`;
  ...
  return <section className="trade-center plan-builder">
    <div className="lab-head">
      <div>
        <p className="eyebrow">Customize Investment Plan · v10.14</p>
        <h2>Capital attack plan</h2>
        <p className="muted">Builds a brave deployment plan from fresh valid records only. Success history and diary remain separate.</p>
      </div>
      <div className="capital-box">
        <span>Total capital</span>
        <div className="capital-input-row"><b>$</b><input type="number" min="1" step="1" value={capital} onChange={e=>setCapital(e.target.value)} /></div>
      </div>
    </div>
    <div className="lab-filters">
      <select value={mode} onChange={e=>setMode(e.target.value)}>
        ...
        <option value="EXTREME">Extreme Hunter</option>
      </select>
      <select value={maxPos} onChange={e=>setMaxPos(e.target.value)}>
        <option value="2">Max 2 positions</option>
        <option value="3">Max 3 positions</option>
        <option value="4">Max 4 positions</option>
        <option value="6">Max 6 positions</option>
      </select>
      <button className="secondary" onClick={()=>copyText(planText())}>Copy Plan</button>
    </div>
    <section className="stats lab-stats">
      <div><span>Deploy Now</span><b>${deploy.toFixed(2)}</b></div>
      <div><span>Reserve</span><b>${reserve.toFixed(2)}</b></div>
      <div><span>Positions</span><b>{rows.length}</b></div>
      <div><span>TP1 Potential</span><b>${tp1.toFixed(2)}</b></div>
      <div><span>TP2 Potential</span><b>${tp2.toFixed(2)}</b></div>
      <div><span>Risk If Invalid</span><b>${risk.toFixed(2)}</b></div>
      <div><span>Diary Active</span><b>{investments.filter(x=>x.status!=="CLOSED").length}</b></div>
    </section>
    <div className="table-wrap">
      <table className="tracker-table action-table">
        <thead><tr><th>Priority</th><th>Action</th><th>Pair</th><th>Bucket</th><th>Size</th><th>TP/SL Binance Fields</th><th>Route</th><th>Reason</th></tr></thead>
        <tbody>{rows.length===0?<tr><td colSpan="8" className="empty-cell">No fresh valid plan now. Wait for next sync.</td></tr>:rows.map((r,i)=>{const a=r.advisory, os=orderSetupFromRecord(r,a,capital);return <tr key={r.id}>
          <td><b>#{i+1}</b><small>Rank {a.priorityScore}</small></td>
          <td><span className={`aggressive-pill ${String(a.agLabel).includes("GRAB")?"grab":String(a.agLabel).includes("ATTACK")?"enter":"scout"}`}>{a.agLabel}</span></td>
          <td><a href={r.url} target="_blank" rel="noreferrer">{r.pairLabel}</a><small>{r.chain} · {r.dex}</small></td>
          <td><span className="bucket-pill">{a.bucket}</span></td>
          <td><b>${n(a.agSize).toFixed(2)}</b><small>of ${n(capital).toFixed(0)}</small></td>
          <td><div className="binance-fields"><b>TP Limit {orderPrice(os.tp1)}</b><small>SL Trigger {orderPrice(os.stopTrigger)}</small><small>SL Price {orderPrice(os.stopLimit)}</small><small>Amount {os.qty.toPrecision(6)} {r.token}</small></div></td>
          <td><a className="action-link" href={a.routeUrl} target="_blank" rel="noreferrer">{a.actionLabel}</a><small>{a.routeMode} → {a.routePair}</small></td>
          <td>{a.agReason}</td>
        </tr>})}</tbody>
      </table>
    </div>
...
function App(){
  ...
  function process(pairs,q){const a=(pairs||[]).map(p=>{const x=analyze(p,q,opts); x.security=assessSecurity(x,opts.securityMode); x.category=getCategory(x); return x}).filter(x=>showRejected?true:(x.decision!=="FILTERED" && securityAllowsTracking(x,opts.securityMode)));const m=new Map();a.forEach(x=>{const key=`${String(x.token).toUpperCase()}-${x.signalType}`;const e=m.get(key);if(!e||rank(x)>rank(e))m.set(key,x)});return[...m.values()].sort((a,b)=>rank(b)-rank(a)).slice(0,90)}
  ...
  async function radar(){setLoading(true);setStatus("Radar scanning mixed buy/sell...");try{let all=[];for(const t of RADAR){const r=await fetch(`https://api.dexscreener.com/latest/dex/search?q=${encodeURIComponent(t)}`);if(!r.ok)continue;const d=await r.json();all.push(...process(d.pairs,t).slice(0,3))}const m=new Map();all.forEach(x=>{const key=`${String(x.token).toUpperCase()}-${x.signalType}`;const e=m.get(key);if(!e||rank(x)>rank(e))m.set(key,x)});const ranked=[...m.values()].sort((a,b)=>rank(b)-rank(a)).slice(0,90);setItems(ranked);setSelected(ranked[0]||null);track(ranked,"radar");setStatus(`Extreme radar found ${ranked.length} fresh mixed opportunities`);setLastSynced(new Date().toISOString())}catch(e){setStatus(`Radar failed: ${e.message}`)}finally{setLoading(false)}}
  function exp(){const blob=new Blob([JSON.stringify({version:"v10.14.1-zero-list-hotfix",exportedAt:new Date().toISOString(),records:ref.current},null,2)],{type:"application/json"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="snitch-x-v10-14-1-zero-list-hotfix.json";a.click();URL.revokeObjectURL(url)}
  ...
  return <main><header className="hero"><div><p className="eyebrow">SNITCH X · Extreme Hunter Full v10.14.1</p><h1>Aggressive opportunity hunter + capital deployment plan.</h1><p className="sub">V10.14.1 adds zero-list fallback so the site does not stay empty when strict validation finds no records.</p></div><div className="hero-card sync-card">
      <span>Status</span>
      ...
      <div className="mode-row"><select value={hunterMode} onChange={e=>setHunterMode(e.target.value)}><option value="BALANCED">Balanced</option><option value="AGGRESSIVE">Aggressive</option><option value="EXTREME">Extreme Hunter</option></select></div>
