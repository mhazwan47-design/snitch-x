...
function binanceTradeUrl(token, quote="USDT"){
...
function routeInfo(r){
  ...
  const major=["WETH","WBTC","WBNB","ETH","BTC","BNB","SOL","WSOL"].includes(q);
  const originalPair=`${token}/${q||"USDT"}`;

  if(q==="USDC"){
    if(isConfirmedBinanceUsdc(token)){
      return {routeMode:"Binance Direct", routePair:`${token}/USDC`, routeUrl:binanceTradeUrl(token,"USDC"), routeButton:`Open Binance ${token}/USDC`, routePenalty:0, routeWarning:"Confirmed USDC route"};
    }
    return {routeMode:"Binance Mirror", routePair:`${token}/USDT`, routeUrl:binanceTradeUrl(token,"USDT"), routeButton:`Open Binance ${token}/USDT Mirror`, routePenalty:80, routeWarning:`${originalPair} not confirmed on Binance. Fallback to ${token}/USDT.`};
  }

  if(q==="USDT"){
    return {routeMode:"Binance Direct", routePair:`${token}/USDT`, routeUrl:binanceTradeUrl(token,"USDT"), routeButton:`Open Binance ${token}/USDT`, routePenalty:0, routeWarning:"Confirmed USDT route"};
  }

  if(stable){
    return {routeMode:"Binance Mirror", routePair:`${token}/USDT`, routeUrl:binanceTradeUrl(token,"USDT"), routeButton:`Open Binance ${token}/USDT Mirror`, routePenalty:80, routeWarning:`Stable quote ${q} fallback to USDT.`};
  }

  if(major){
    return {routeMode:"Binance Mirror", routePair:`${token}/USDT`, routeUrl:binanceTradeUrl(token,"USDT"), routeButton:`Open Binance ${token}/USDT Mirror`, routePenalty:80, routeWarning:`Original scanner pair is ${r?.pairLabel}; Binance opens ${token}/USDT mirror.`};
  }
