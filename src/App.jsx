...
function TradeActionCenter({ records, investments, onMarkInvestment }){
  ...
  return (
    <section className="trade-center">
      <div className="lab-head">
        <div>
          <p className="eyebrow">Trade Action Center · v10.10 Route Fix</p>
        ...
        <tbody>
          {rows.length===0 ? <tr><td colSpan="11" className="empty-cell">No investment diary yet. Use Manual Add or mark ★ from Trade Action Center after you execute in Binance.</td></tr> : rows.map(inv => (
            <tr key={inv.diaryId}>
              ...
              <td><a href={inv.url || platformTradeUrl("Binance", inv.token)} target="_blank" rel="noreferrer">{inv.pairLabel}</a><small>{inv.actionLabel} · Rank {inv.priorityScore}</small></td>
...
export default function App(){
  ...
  function addManualInvestment(form){
    ...
    const item = {
      ...
      createdAt: now,
      updatedAt: now,
      notes: "Manually added from Binance holding."
    };
  ...
  function exp(){const blob=new Blob([JSON.stringify({version:"v10.10-binance-route-fix",exportedAt:new Date().toISOString(),records:ref.current},null,2)],{type:"application/json"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="snitch-x-v10-10-binance-route-fix.json";a.click();URL.revokeObjectURL(url)}
  ...
  return <main><header className="hero"><div><p className="eyebrow">SNITCH X · Binance Route Fix v10.10</p><h1>Binance direct/mirror route fixed.</h1><p className="sub">V10.10 fixes Binance links: Direct vs Mirror vs Chart Only, with duplicate route reduction.</p></div><div className="hero-card sync-card">
      <span>Status</span>
