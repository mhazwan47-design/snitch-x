...
function actionFor(r, capital=10, mode="EXTREME"){
  ...
  if(met.tp1Pct < Math.max(3.2, cost+2.2)) return {...met, label:"", size:0, score:0, reason:"Hidden: not enough net profit room after cost.", bucket:b};
  if(risk<=0 || risk>18) return {...met, label:"", size:0, score:0, reason:"Hidden: risk gate failed.", bucket:b};
  if(sec<62) return {...met, label:"", size:0, score:0, reason:"Hidden: security/quality gate failed.", bucket:b};
  let score = 55 + sec*0.55 + n(r.execution)*0.35 + n(r.opportunity)*0.35 - n(r.risk)*0.28 + met.tp1Pct*2.0 - cost*4.0;
  if(r.hitTrigger) score+=14;
  if(r.hitZone) score+=8;
  if(marketLabel(r)==="SPOT") score+=5;
  if(marketLabel(r)==="PERP"||marketLabel(r)==="FUTURES") score+=3;
  if(m==="BALANCED" && risk>8) score-=12;
  if(m==="AGGRESSIVE") score+=5;
  if(m==="EXTREME") score+=8;
  ...
  return {...met, label, size, score:Math.round(score), reason:`Passed hidden fee/room/liquidity/risk filter. Est. cost ${cost.toFixed(2)}%.`, bucket:b};
}
function orderFields(r, size){
  const entry=n(r.currentPrice||r.signalPrice);
  const qty=entry>0?n(size)/entry:0;
  const isSell=String(r?.signalType||"").toUpperCase()==="SELL";
  const stopPct=marketLabel(r)==="PERP"||marketLabel(r)==="FUTURES"?0.018:marketLabel(r)==="MARGIN"?0.024:0.02;
  const slTrig=isSell?entry*(1+stopPct):entry*(1-stopPct);
  const slPrice=isSell?entry*(1+stopPct+0.0045):entry*(1-stopPct-0.0045);
  return {tp:n(r.tp1), tp2:n(r.tp2), slTrig, slPrice, qty};
}
...
function Header({status, sync}){
  return <header className="hero">
    <div>
      <p className="eyebrow">SNITCH X · Investment Lock + Success Intel v10.18</p>
      <h1>Extreme Hunter investment lock active</h1>
      <p className="sub">Auto-removes marked diary coins from Trade Action, keeps fresh invalid cleanup, and builds Success Forecast intelligence dashboard.</p>
    </div>
...
function TradeAction({records, investments, setInvestments}){
  ...
  const rows=useMemo(()=>{
    ...
    records.filter(r=>stillValid(r)&&!isInDiary(r,investments)).map(r=>{
      ...
      const rt=routeInfo(r);
      const curatedBoost=(CURATED_LIST_PAIRS.has(String(r.pairLabel||"").toUpperCase())||isCuratedToken(tokenOf(r)))?35:0; const score=a.score + n(r.securityScore) + curatedBoost + (rt.routeMode==="Binance Direct"?40:rt.routeMode==="Binance Mirror"?10:-80) - n(rt.routePenalty||0);
      return {...r,a,rt,score};
...
Route: ${r.rt.routeMode} ${r.rt.routePair}`;}).join("\n\n");
  ...
  return <section className="trade-center">
    <div className="lab-head"><div><p className="eyebrow">SNITCH Execution Now</p><h2>Tradable only</h2><p className="muted">Hidden filters remove weak setups. Visible labels are direct: BUY/SELL + market.</p></div><div className="capital-box"><span>Capital</span><div className="capital-input-row"><b>$</b><input type="number" value={capital} onChange={e=>setCapital(e.target.value)}/></div></div></div>
    <Guides/>
