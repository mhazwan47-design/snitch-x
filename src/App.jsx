...
function orderSetupFromRecord(record, advisory, capital){
  ...
  return {
    ...
    stopText:`Stop: ${orderPrice(practicalStop)} | Price: ${orderPrice(stopLimit)} | Amount: MAX / ${qty.toPrecision(6)}`,
    tpText:`TP1: ${orderPrice(tp1)} for 50% | TP2: ${orderPrice(tp2)} for balance`
  };
}
...
function TradeActionCenter({ records, investments, onMarkInvestment }){
  ...
  return (
    <section className="trade-center">
      <div className="lab-head">
        <div>
          <p className="eyebrow">Trade Action Center · v10.7 Order Setup</p>
            ...
            {rows.length === 0 ? (
              ...
              return (
                <tr key={`${r.id}-action`}>
                  ...
                  <td>{(()=>{const os=orderSetupFromRecord(r,a,capital); return <><b>Stop {orderPrice(os.stopTrigger)}</b><small>Price {orderPrice(os.stopLimit)}</small><small>Amount {os.qty.toPrecision(6)}</small></>})()}</td>
                  <td>
...
function InvestmentDiary({ investments, records, onCloseInvestment, onRemoveInvestment }){
  ...
  return <section className="investment-diary">
    <div className="lab-head">
      <div>
        <p className="eyebrow">Investment Diary · Complete Order Tracker</p>
    ...
    <div className="table-wrap">
      <table className="tracker-table investment-table">
        ...
        <tbody>
          {rows.length===0 ? <tr><td colSpan="11" className="empty-cell">No investment diary yet. Mark ★ from Trade Action Center after you manually execute in Binance.</td></tr> : rows.map(inv => (
            <tr key={inv.diaryId}>
              ...
              <td>
                <b>Stop {orderPrice(inv.stopTrigger || inv.entryPrice*0.98)}</b>
                <small>Price {orderPrice(inv.stopLimit || (inv.entryPrice*0.978))}</small>
                <small>Amount MAX / {n(inv.quantity).toPrecision(6)}</small>
                <button className="mini-btn" onClick={()=>copyText(`STOP LIMIT ${inv.pairLabel}\nStop: ${orderPrice(inv.stopTrigger || inv.entryPrice*0.98)}\nPrice: ${orderPrice(inv.stopLimit || inv.entryPrice*0.978)}\nAmount: ${n(inv.quantity).toPrecision(6)} ${inv.token}\nTP1: ${orderPrice(inv.tp1)}\nTP2: ${orderPrice(inv.tp2)}`)}>Copy setup</button>
              </td>
...
export default function App(){
  ...
  function exp(){const blob=new Blob([JSON.stringify({version:"v10.7-complete-order-setup",exportedAt:new Date().toISOString(),records:ref.current},null,2)],{type:"application/json"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="snitch-x-v10-7-complete-order-setup.json";a.click();URL.revokeObjectURL(url)}
  ...
  return <main><header className="hero"><div><p className="eyebrow">SNITCH X · Complete Order Setup v10.7</p><h1>Priority ranking + complete stop-limit setup.</h1><p className="sub">V10.7 adds practical stop trigger, stop limit price, amount and TP guidance.</p></div><div className="hero-card sync-card">
      <span>Status</span>
