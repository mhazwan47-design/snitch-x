...
function routeQualityForPair(record){
  ...
  const stable = ["USDT","USDC","FDUSD","BUSD"].includes(q);
  const majorQuote = ["WETH","WBTC","WBNB","ETH","BTC","BNB"].includes(q);
  const routePair = stable ? `${token}/${q === "USDC" ? "USDC" : "USDT"}` : `${token}/USDT`;
  const symbol = routePair.replace("/", "");
  if(stable){
    return {routeMode:"Binance Direct",routePair,routeWarning:"",routeUrl:binanceTradeUrlForSymbol(symbol),routePenalty:0,routeButton:`Open Binance ${routePair}`};
  }
  if(majorQuote){
    return {routeMode:"Binance Mirror",routePair,routeWarning:`Original scanner pair is ${record.pairLabel}; Binance route opens ${routePair} as mirror.`,routeUrl:binanceTradeUrlForSymbol(symbol),routePenalty:80,routeButton:`Open Binance ${routePair} Mirror`};
  }
  return {routeMode:"Chart Only",routePair:record.pairLabel,routeWarning:"No safe Binance spot mirror detected. Use chart first.",routeUrl:record.url || "#",routePenalty:180,routeButton:"Open Chart"};
}
...
function routeAdvisor(record, capital){
  ...
  const rq = routeQualityForPair(record); return { ...pnl, ...beginnerLabelForRecord(record), ...aggressivePriorityForRecord(record, capital), route, platform, actionLabel: rq.routeButton || actionLabel, routeUrl: rq.routeUrl || routeUrl, viability, reason: rq.routeWarning ? `${reason} ${rq.routeWarning}` : reason, feeTier:fee.feeTier, ...rq };
}
...
function SuccessForecast({ records }){
  ...
  const rows = useMemo(()=>{
    let a = usefulLabRecords(records).filter(r => r.isSuccessArchive || r.hitTP1 || r.hitTP2 || String(r.resultStatus||"").includes("SUCCESS"));
...
export default 
function safeRouteForPlan(r){
  try {
    if(typeof routeForAnyHolding === "function") return routeForAnyHolding(r.pairLabel, r.token, r.url);
    if(typeof routeQualityForPair === "function") return routeQualityForPair(r);
  } catch(e) {}
  const token = normalizeTicker(r?.token || String(r?.pairLabel||"").split("/")[0] || "");
  return {
    routeMode: "Binance Direct",
    routePair: `${token}/USDT`,
    routeUrl: platformTradeUrl("Binance", token),
    routeButton: `Open Binance ${token}/USDT`,
    routeWarning: ""
  };
}
function safeBucket(r){
  const t = String(r?.token||"").toUpperCase();
  const high = new Set(["PEPE","WIF","BONK","FLOKI","SHIB","1000SATS","1000CAT","ACT","PNUT","MEME","TURBO","BOME","MEW","POPCAT","MOG","GOAT","PENGU","DOGS","NOT"]);
  const narrative = new Set(["AI","AIXBT","VIRTUAL","GRASS","KAITO","BERA","ZRO","AEVO","EIGEN","ALT","ARKM","CETUS","DEEP"]);
  if(high.has(t)) return "HIGH VOL";
  if(narrative.has(t)) return "NARRATIVE";
  if(CEX_TOKEN_SET?.has?.(t)) return "CEX SAFE";
  return "WILD CARD";
}
function safeActionLabel(r, mode){
  const bucket = safeBucket(r);
  const sec = n(r.securityScore || r.security?.securityScore);
  const p = n(r.currentPrice || r.signalPrice), tp2 = n(r.tp2), tp1=n(r.tp1), inv=n(r.invalidation);
  const tp2pct = p && tp2 ? ((tp2-p)/p)*100 : 0;
  const risk = p && inv ? Math.abs(((inv-p)/p)*100) : 99;
  if(r.signalType === "SELL") return "SKIP / SELL WATCH";
  if(sec >= 95 && r.hitTrigger && tp2pct >= 4 && risk <= 16) return "GRAB NOW";
  if(String(mode).includes("EXTREME") && bucket === "HIGH VOL" && sec >= 65 && tp2pct >= 6) return "HIGH VOL SCOUT";
  if(String(mode).includes("EXTREME") && bucket === "NARRATIVE" && sec >= 65 && tp2pct >= 6) return "ATTACK ENTRY";
  if(sec >= 85 && tp2pct >= 5) return "ENTER SMALL";
  return "SCOUT ENTRY";
}
function safeSize(capital, label){
  const cap=n(capital,10);
  if(String(label).includes("GRAB")) return Math.min(cap, cap*0.7);
  if(String(label).includes("ATTACK")) return Math.min(cap, cap*0.45);
  if(String(label).includes("HIGH VOL")) return Math.min(cap, cap*0.25);
  if(String(label).includes("ENTER")) return Math.min(cap, cap*0.4);
  return Math.min(cap, cap*0.2);
}
function SafeCustomizeInvestmentPlan({records, investments}){
  const [capital,setCapital]=useState(50);
  const [mode,setMode]=useState("EXTREME");
  const [maxPos,setMaxPos]=useState(4);
  const rows = useMemo(()=>{
    const active = (typeof validActionRecords==="function" ? validActionRecords(records) : (records||[]))
      .filter(r=>r && r.signalType==="BUY")
      .map(r=>{
        const route=safeRouteForPlan(r);
        const label=safeActionLabel(r, mode);
        const size=safeSize(capital,label);
        const entry=n(r.currentPrice || r.signalPrice);
        const qty=entry>0 ? size/entry : 0;
        const tp1=n(r.tp1), tp2=n(r.tp2);
        const slTrig=entry*0.9825, slPrice=entry*0.978;
        const score = (n(r.securityScore)*1.2) + (n(r.execution)*0.8) + (n(r.validity)*0.7) + (tp2>entry ? ((tp2-entry)/entry)*100*5 : 0) - (route.routeMode==="Binance Mirror"?80:0);
        return {...r, route, label, size, qty, tp1, tp2, slTrig, slPrice, score, bucket:safeBucket(r)};
      })
      .filter(x=>x.size>0 && !String(x.label).includes("SKIP"))
      .sort((a,b)=>b.score-a.score);
    const byRoute=new Map();
    for(const r of active){
      const key=r.route.routePair || r.pairLabel;
      const old=byRoute.get(key);
      if(!old || r.score>old.score) byRoute.set(key,r);
    }
    return [...byRoute.values()].slice(0,n(maxPos,4));
  },[records,capital,mode,maxPos]);
  const deploy=rows.reduce((s,r)=>s+n(r.size),0);
  const reserve=Math.max(0,n(capital)-deploy);
  const activeDiary=(investments||[]).filter(x=>x.status!=="CLOSED").length;
  function copyPlan(){
    const text = [
      "SNITCH X SAFE INVESTMENT PLAN",
      `Capital: $${n(capital).toFixed(2)}`,
      `Mode: ${mode}`,
      `Deploy: $${deploy.toFixed(2)}`,
      `Reserve: $${reserve.toFixed(2)}`,
      "",
      ...rows.map((r,i)=>`${i+1}. ${r.pairLabel} | ${r.label} | ${r.bucket}
Size: $${n(r.size).toFixed(2)}
Route: ${r.route.routeMode} ${r.route.routePair}
TP Limit: ${orderPrice(r.tp1)}
SL Trigger: ${orderPrice(r.slTrig)}
SL Price: ${orderPrice(r.slPrice)}
Amount: ${r.qty.toPrecision(6)} ${r.token}`)
    ].join("\\n");
    try{navigator.clipboard.writeText(text)}catch{}
  }
  return <section className="trade-center plan-builder">
    <div className="lab-head">
      <div><p className="eyebrow">Customize Investment Plan · v10.15.1 Safe</p><h2>Capital attack plan</h2><p className="muted">Safe standalone plan builder using fresh active records. Built to avoid blank-screen dependency crashes.</p></div>
      <div className="capital-box"><span>Capital</span><div className="capital-input-row"><b>$</b><input type="number" min="1" step="1" value={capital} onChange={e=>setCapital(e.target.value)}/></div></div>
    </div>
    <div className="lab-filters">
      <select value={mode} onChange={e=>setMode(e.target.value)}><option value="BALANCED">Balanced</option><option value="AGGRESSIVE">Aggressive</option><option value="EXTREME">Extreme Hunter</option></select>
      <select value={maxPos} onChange={e=>setMaxPos(e.target.value)}><option value="2">Max 2</option><option value="3">Max 3</option><option value="4">Max 4</option><option value="6">Max 6</option></select>
      <button className="secondary" onClick={copyPlan}>Copy Plan</button>
    </div>
    <section className="stats lab-stats">
      <div><span>Deploy</span><b>${deploy.toFixed(2)}</b></div><div><span>Reserve</span><b>${reserve.toFixed(2)}</b></div><div><span>Positions</span><b>{rows.length}</b></div><div><span>Diary Active</span><b>{activeDiary}</b></div>
    </section>
    <div className="table-wrap"><table className="tracker-table action-table">
      <thead><tr><th>#</th><th>Action</th><th>Pair</th><th>Bucket</th><th>Size</th><th>Binance TP/SL Fields</th><th>Route</th></tr></thead>
      <tbody>{rows.length===0?<tr><td colSpan={7} className="empty-cell">No fresh plan now. Sync/run tracker first.</td></tr>:rows.map((r,i)=><tr key={r.id||i}>
        <td><b>{i+1}</b><small>Score {Math.round(r.score)}</small></td>
        <td><span className={`aggressive-pill ${String(r.label).includes("GRAB")?"grab":String(r.label).includes("ATTACK")||String(r.label).includes("ENTER")?"enter":"scout"}`}>{r.label}</span></td>
        <td><a href={r.url} target="_blank" rel="noreferrer">{r.pairLabel}</a><small>{r.chain} · {r.dex}</small></td>
        <td><span className="bucket-pill">{r.bucket}</span></td>
        <td><b>${n(r.size).toFixed(2)}</b></td>
        <td><div className="binance-fields"><b>TP Limit {orderPrice(r.tp1)}</b><small>SL Trigger {orderPrice(r.slTrig)}</small><small>SL Price {orderPrice(r.slPrice)}</small><small>Amount {r.qty.toPrecision(6)} {r.token}</small></div></td>
        <td><a className="action-link" href={r.route.routeUrl} target="_blank" rel="noreferrer">{r.route.routeButton||"Open Route"}</a><small>{r.route.routeMode} → {r.route.routePair}</small></td>
      </tr>)}</tbody>
    </table></div>
  </section>
}
...
function App(){
  ...
  function exp(){const blob=new Blob([JSON.stringify({version:"v10.15.1-safe-integration",exportedAt:new Date().toISOString(),records:ref.current},null,2)],{type:"application/json"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="snitch-x-v10-15-1-safe-integration.json";a.click();URL.revokeObjectURL(url)}
  ...
  return <main><header className="hero"><div><p className="eyebrow">SNITCH X · Safe Extreme Integration v10.15.1</p><h1>Stable UI + safe plan builder. Expanded worker active.</h1><p className="sub">V10.15.1 re-adds Customize Investment Plan as a safe standalone tab without breaking the stable UI.</p></div><div className="hero-card sync-card">
      <span>Status</span>
