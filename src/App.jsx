...
export default function App(){
  const [query,setQuery]=useState(""),[items,setItems]=useState([]),[selected,setSelected]=useState(null),[status,setStatus]=useState("Ready"),[loading,setLoading]=useState(false),[tab,setTab]=useState("scanner"),[records,setRecords]=useState([]),[remote,setRemote]=useState(false),[syncing,setSyncing]=useState(false),[lastSynced,setLastSynced]=useState(null),[syncError,setSyncError]=useState(""),[minLiquidity,setMinLiquidity]=useState(200000),[strictness,setStrictness]=useState("balanced"),[securityMode,setSecurityMode]=useState("balanced"),[showRejected,setShowRejected]=useState(false);
  ...
  function exp(){const blob=new Blob([JSON.stringify({version:"v10.4-all-tabs-auto-sync",exportedAt:new Date().toISOString(),records:ref.current},null,2)],{type:"application/json"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="snitch-x-v10-4-auto-sync-tracker.json";a.click();URL.revokeObjectURL(url)}
  ...
  return <main><header className="hero"><div><p className="eyebrow">SNITCH X · All Tabs Auto Sync v10.4</p><h1>Live scanner, lab, success and action center.</h1><p className="sub">V10.4 auto-syncs every tab, including Scanner, from the remote tracker DB.</p></div><div className="hero-card sync-card">
      <span>Status</span>
