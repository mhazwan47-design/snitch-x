...
function TradeActionCenter({ records, investments, onMarkInvestment }){
  ...
  return (
    <section className="trade-center">
      <div className="lab-head">
        <div>
          <p className="eyebrow">Trade Action Center · v10.9 Binance Terms</p>
            ...
            {rows.length === 0 ? (
              ...
              return (
                <tr key={`${r.id}-action`}>
                  ...
                  <td>{(()=>{const os=orderSetupFromRecord(r,a,capital); return <div className="binance-fields"><b>TP Limit {orderPrice(os.tp1)}</b><small>SL Trigger {orderPrice(os.stopTrigger)}</small><small>SL Price {orderPrice(os.stopLimit)}</small><small>Amount {os.qty.toPrecision(6)} {r.token}</small></div>})()}</td>
                  <td>
...
export default function App(){
  ...
  function exp(){const blob=new Blob([JSON.stringify({version:"v10.9-binance-terms-sync",exportedAt:new Date().toISOString(),records:ref.current},null,2)],{type:"application/json"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="snitch-x-v10-9-binance-terms-sync.json";a.click();URL.revokeObjectURL(url)}
  ...
  return <main><header className="hero"><div><p className="eyebrow">SNITCH X · Binance Terms Sync v10.9</p><h1>Binance-ready TP/SL and order terminology.</h1><p className="sub">V10.9 uses Binance wording: TP Limit, SL Trigger, SL Price, Amount, Open Orders.</p></div><div className="hero-card sync-card">
      <span>Status</span>
