...
function scannerItemsFromRecords(records){
  return validActionRecords(records)
    .sort((a,b) => {
...
function validActionRecords(records){
  return pruneRecordsForStorage(records).filter(isActiveTradeValid);
}
...
function TradeActionCenter({ records, investments, onMarkInvestment }){
  ...
  return (
    <section className="trade-center">
      <div className="lab-head">
        <div>
          <p className="eyebrow">Trade Action Center · v10.12 Valid Only</p>
      ...
      <BinanceTermsGuide />
      <RouteSafetyGuide />
      <div className="lab-note"><b>Validity gate:</b> rows that already passed TP1/TP2, hit invalidation, are stale, or have too little remaining TP room are hidden automatically.</div>
...
export default function App(){
  ...
  function exp(){const blob=new Blob([JSON.stringify({version:"v10.12.1-worker-async-patch",exportedAt:new Date().toISOString(),records:ref.current},null,2)],{type:"application/json"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="snitch-x-v10-12-1-worker-async-patch.json";a.click();URL.revokeObjectURL(url)}
  ...
  return <main><header className="hero"><div><p className="eyebrow">SNITCH X · Valid List Cleanup v10.12.1</p><h1>Only valid active setups, success records and diary are kept.</h1><p className="sub">V10.12.1 fixes GitHub worker async syntax and keeps valid-list cleanup active.</p></div><div className="hero-card sync-card">
      <span>Status</span>
