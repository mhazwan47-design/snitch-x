...
function routeAdvisor(record, capital){
  ...
  return { ...pnl, ...beginnerLabelForRecord(record), route, platform, actionLabel, routeUrl, viability, reason, feeTier:fee.feeTier };
}
...
function TradeActionCenter({ records }){
  ...
  const rows = useMemo(() => {
    const base = (records || []).map(r => ({ ...r, advisory: routeAdvisor(r, capital) }));
    let filtered = base;
    if(filter === "BUY") filtered = base.filter(r => r.signalType === "BUY");
    if(filter === "SELL") filtered = base.filter(r => r.signalType === "SELL");
    ...
    if(filter === "ACTIONABLE") filtered = base.filter(r => r.signalType === "BUY" && !String(r.category||"").includes("WAIT") && n(r.securityScore) > 0 && !String(r.advisory.viability).includes("AVOID") && !String(r.advisory.viability).includes("NOT WORTH"));
  ...
  return (
    <section className="trade-center">
      <div className="lab-head">
        <div>
          <p className="eyebrow">Trade Action Center · v9.1</p>
          <h2>Capital calculator + route advisor</h2>
          <p className="muted">
            Enter capital once. SNITCH X calculates TP profit, invalidation risk, fee suitability, best route, and action buttons.
          </p>
        </div>
        <div className="capital-box">
          <span>Capital / modal</span>
      ...
      <section className="stats lab-stats">
        ...
        <div><span>Capital Each</span><b>${n(capital).toFixed(0)}</b></div>
      ...
      <div className="table-wrap">
        <table className="tracker-table action-table">
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr><td colSpan="9" className="empty-cell">No rows for this filter.</td></tr>
            ) : rows.map((r) => {
              const a = r.advisory;
              return (
                <tr key={`${r.id}-action`}>
                  <td><span className={`viability-pill ${String(a.viability).includes("BEST")?"best":String(a.viability).includes("OK")?"ok":String(a.viability).includes("PAPER")?"paper":String(a.viability).includes("AVOID")?"avoid":"watch"}`}>{a.viability}</span></td>
                  <td><span className={`simple-signal-pill ${String(a.beginnerAction).includes("BUY NOW")?"buy":String(a.beginnerAction).includes("WAIT")?"wait":String(a.beginnerAction).includes("TOO LATE")||String(a.beginnerAction).includes("AVOID")||String(a.beginnerAction).includes("CUT")?"avoid":String(a.beginnerAction).includes("TAKE")?"profit":"watch"}`}>{a.beginnerAction}</span><small>{a.beginnerWhy}</small></td>
                  <td>
...
export default function App(){
  ...
  return <main><header className="hero"><div><p className="eyebrow">SNITCH X · Beginner Signal Mode v10.3</p><h1>Simple buy / wait / avoid / profit signals.</h1><p className="sub">V10.3 makes alerts beginner-friendly: BUY NOW SMALL, WAIT, TOO LATE, AVOID, TAKE PROFIT, or CUT LOSS.</p></div><div className="hero-card"><span>Status</span><b>{loading?"Scanning...":status}</b></div></header><section className="tabbar"><button className={tab==="scanner"?"tab active":"tab"} onClick={()=>setTab("scanner")}>Scanner</button><button className={tab==="lab"?"tab active":"tab"} onClick={()=>setTab("lab")}>Performance Lab</button><button className={tab==="success"?"tab active":"tab"} onClick={()=>setTab("success")}>Success Forecast</button><button className={tab==="action"?"tab active":"tab"} onClick={()=>setTab("action")}>Trade Action Center</button></section>{tab==="scanner"?<><section className="toolbar"><input value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>e.key==="Enter"&&search()} placeholder="Search token e.g. ETH, PEPE"/><select value={minLiquidity} onChange={e=>setMinLiquidity(Number(e.target.value))}><option value="0">No liquidity filter</option><option value="50000">Min liquidity $50K</option><option value="200000">Min liquidity $200K</option><option value="1000000">Min liquidity $1M</option></select><select value={strictness} onChange={e=>setStrictness(e.target.value)}><option value="hunter">hunter</option><option value="balanced">balanced</option><option value="strict">strict</option></select><select value={securityMode} onChange={e=>setSecurityMode(e.target.value)}><option value="loose">security loose</option><option value="balanced">security balanced</option><option value="strict">security strict</option></select><button onClick={()=>search()} disabled={loading}>Search</button><button className="secondary" onClick={radar} disabled={loading}>Run Radar</button></section><section className="toggle-row"><label><input type="checkbox" checked={showRejected} onChange={e=>setShowRejected(e.target.checked)}/>Show rejected / bad-data pairs</label></section><section className="stats"><div><span>Pairs</span><b>{items.length}</b></div><div><span>Buy Watch</span><b>{stats.buy}</b></div><div><span>Sell Watch</span><b>{stats.sell}</b></div><div><span>Tracked DB</span><b>{records.length}</b></div><div><span>Avg Execution</span><b>{stats.avg}</b></div></section><section className="layout"><div className="list">{items.length===0?<div className="empty"><h2>No signals loaded yet</h2><p>Run Radar to collect mixed BUY WATCH and SELL WATCH records.</p></div>:items.map(it=><Card key={it.id} item={it} selected={selected?.id===it.id} onSelect={setSelected}/>)}</div><Detail item={selected}/></section></>:tab==="lab"?<Lab records={records} remoteLoaded={remote} onExport={exp} onImport={imp} onClear={clr} onSync={syncRemote}/>:tab==="success"?<SuccessForecast records={records}/>:<TradeActionCenter records={records}/>}</main>
}
