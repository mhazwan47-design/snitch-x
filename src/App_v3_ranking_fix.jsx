...
function analyzePair(pair, query, opts) {
  ...
  const cleanEnough = !rejection && majorQuote && strongActivity;
  ...
  if (rejection && opts.hideRejected !== false) {
    ...
    size = "0%";
  } else if (cleanEnough && execution >= 78 && validity >= 70 && risk <= 58 && rr >= 0.55) {
    decision = "BUY SMALL";
...
function finalRank(x) {
  const decisionBoost =
    x.decision === "BUY SMALL" ? 42 :
    x.decision === "WATCH HOT" ? 24 :
    x.decision === "WAIT" ? 8 :
  ...
  return (
    <section className="panel sticky-panel">
      ...
      <div className="rule-box">
        <h4>V3 Ranking Fix</h4>
        <p><b>Priority:</b> major quote pair, clean volume, real transaction count, major DEX/chain, normal price versus reference pairs.</p>
        <p><b>Rejects/penalizes:</b> weak volume, non-major quote, huge liquidity with dead volume, price anomaly, FDV/liquidity imbalance, extreme pump/dump.</p>
  ...
  return (
    <main>
      <header className="hero">
        <div>
          <p className="eyebrow">SNITCH X · Universal DEX Execution Cockpit v3</p>
          <h1>Prioritize the real pair, not the noisy pair.</h1>
          <p className="sub">
        ...
        <div className="list">
          {items.length === 0 ? (
            <div className="empty">
              ...
              <p>Search a token or run Radar. V3 prioritizes major quote pairs and penalizes noisy or abnormal pairs.</p>
            </div>
          ) : items.map(item => (
