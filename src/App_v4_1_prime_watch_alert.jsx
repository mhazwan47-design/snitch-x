...
function buildAlertText(x) {
  const current = `$${price(x.price)}`;
  const buyZone = `$${price(x.buyLow)} - $${price(x.buyHigh)}`;
  const breakout = `$${price(x.breakout)}`;
  const stop = `$${price(x.invalidation)}`;
  const tp = `$${price(x.tp1)} / $${price(x.tp2)}`;

  if (x.decision === "BUY SMALL") {
    return `SNITCH BUY TRIGGER: ${x.pairLabel} on ${x.chain}/${x.dex}
Decision: BUY SMALL
Current: ${current}
Buy Zone: ${buyZone}
Breakout: ${breakout}
...
TP1/TP2: ${tp}
RR: 1:${x.rr.toFixed(2)}
Risk: ${Math.round(x.risk)}
Action: small size only after candle confirmation.
Chart: ${x.url}`;
  }

  if (x.decision === "PRIME WATCH") {
    return `SNITCH PRIME WATCH: ${x.pairLabel} on ${x.chain}/${x.dex}
Current: ${current}
Wait for:
1) Pullback to ${buyZone}
OR
2) Breakout/retest above ${breakout}
Invalidation: ${stop}
TP1/TP2: ${tp}
RR now: 1:${x.rr.toFixed(2)}
Action: no buy now. Set alert.
Chart: ${x.url}`;
  }

  if (x.decision.includes("WAIT")) {
    return `SNITCH WAIT: ${x.pairLabel} on ${x.chain}/${x.dex}
Current: ${current}
Reason: ${x.decision}
Better buy zone: ${buyZone}
Breakout trigger: ${breakout}
Invalidation: ${stop}
Action: wait. No market buy.
Chart: ${x.url}`;
  }

  return `SNITCH ${x.decision}: ${x.pairLabel} on ${x.chain}/${x.dex}
Current: ${current}
Action: ${x.decision}
Chart: ${x.url}`;
}
...
function analyzePair(pair, query, opts) {
  ...
  if (rejection && opts.hideRejected !== false) {
    ...
    action = priceNearBuyZone
      ? "Buy trigger armed: only enter small if candle confirms bounce inside/near buy zone."
      : "Breakout trigger armed: only enter small after breakout and retest confirmation.";
    size = "0.5% - 1.0% capital";
  } else if (primeWatch && rr < 0.8) {
    decision = "PRIME WATCH";
    action = "High-quality pair, but RR is not ready. Set alert for buy zone or breakout retest.";
    size = "0% now";
  } else if (primeWatch && priceExtendedAboveBuy) {
    decision = "PRIME WATCH";
    action = "Strong candidate, but price is mid-range/extended. Wait for pullback or breakout retest.";
    size = "0% now";
  } else if (cleanEnough && rr < 0.8 && (execution >= 74 || opportunity >= 68)) {
    decision = "WAIT BETTER RR";
    action = "Good pair, but reward/risk is not attractive enough. Wait for a better entry price.";
    size = "0% now";
  } else if (cleanEnough && priceExtendedAboveBuy && (execution >= 74 || opportunity >= 68)) {
...
function finalRank(x) {
  const decisionBoost =
    x.decision === "BUY SMALL" ? 52 :
    x.decision === "PRIME WATCH" ? 36 :
...
function decisionClass(decision) {
  ...
  if (d.includes("HOT") || d.includes("PRIME")) return "hot";
  return "wait";
}
  ...
  return (
    <section className="panel sticky-panel">
      ...
      <div className="rule-box">
        ...
        <p><b>PRIME WATCH:</b> valid and active pair, but price/RR is not ready. This is the best candidate for Telegram watch alert.</p>
      ...
      <div className="rule-box">
        ...
        <pre className="alert-preview">{item.alertText}</pre>
      </div>
...
export default function App() {
  ...
  const stats = useMemo(() => {
    ...
    const hot = items.filter(x => x.decision.includes("HOT") || x.decision.includes("PRIME")).length;
  ...
  return (
    <main>
      <header className="hero">
        <div>
          <p className="eyebrow">SNITCH X · Universal DEX Execution Cockpit v4.1</p>
          <h1>Wait smart, alert early, buy only when the gate opens.</h1>
          <p className="sub">
      ...
      <section className="stats">
        ...
        <div><span>Prime Watch</span><b>{stats.hot}</b></div>
